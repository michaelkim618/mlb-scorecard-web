// Auto-generated (scorecard) — 2026-09-22
window.PREDICTIONS_DATA = [
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "10:05 AM PT",
    "game_number": 1,
    "doubleheader": true,
    "game_pk": 823543,
    "away": "Tampa Bay Rays",
    "home": "New York Yankees",
    "away_standing": {
      "div_rank": 1,
      "div_name": "AL East",
      "wins": 96,
      "losses": 61,
      "games_back": "-"
    },
    "home_standing": {
      "div_rank": 2,
      "div_name": "AL East",
      "wins": 90,
      "losses": 67,
      "games_back": "6.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "4-3",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "10-1",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-4",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "2-1",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "5-1",
          "home": true
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "W",
          "score": "8-1",
          "home": false
        },
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "4-5",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "9-2",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "3-5",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "4-8",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -2
    },
    "away_pitcher": "Nick Martinez",
    "away_pitcher_id": 607259,
    "away_pitcher_stats": {
      "wins": 15,
      "losses": 5,
      "era": "2.94"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-31",
        "opp": "vs CWS",
        "decision": "L",
        "ip": "6.0",
        "er": 6,
        "h": 7,
        "bb": 0,
        "so": 2,
        "era": 9.0
      },
      {
        "date": "2026-08-05",
        "opp": "@ COL",
        "decision": "W",
        "ip": "5.2",
        "er": 0,
        "h": 6,
        "bb": 0,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-08-11",
        "opp": "@ OAK",
        "decision": "W",
        "ip": "9.0",
        "er": 4,
        "h": 8,
        "bb": 0,
        "so": 5,
        "era": 4.0
      },
      {
        "date": "2026-08-18",
        "opp": "vs TOR",
        "decision": "L",
        "ip": "3.2",
        "er": 6,
        "h": 9,
        "bb": 2,
        "so": 5,
        "era": 14.73
      },
      {
        "date": "2026-08-23",
        "opp": "@ BAL",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-08-29",
        "opp": "vs SD",
        "decision": null,
        "ip": "6.0",
        "er": 3,
        "h": 7,
        "bb": 0,
        "so": 4,
        "era": 4.5
      },
      {
        "date": "2026-09-04",
        "opp": "@ TEX",
        "decision": "W",
        "ip": "6.1",
        "er": 3,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 4.26
      },
      {
        "date": "2026-09-10",
        "opp": "@ ATL",
        "decision": null,
        "ip": "6.1",
        "er": 0,
        "h": 2,
        "bb": 0,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-09-16",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 5,
        "bb": 0,
        "so": 3,
        "era": 3.6
      },
      {
        "date": "2026-09-22",
        "opp": "@ NYY",
        "decision": "L",
        "ip": "6.0",
        "er": 2,
        "h": 5,
        "bb": 1,
        "so": 7,
        "era": 3.0
      }
    ],
    "home_pitcher": "Carlos Rodón",
    "home_pitcher_id": 607074,
    "home_pitcher_stats": {
      "wins": 7,
      "losses": 3,
      "era": "2.67"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-06-17",
        "opp": "vs CWS",
        "decision": "W",
        "ip": "5.0",
        "er": 3,
        "h": 7,
        "bb": 1,
        "so": 7,
        "era": 5.4
      },
      {
        "date": "2026-06-23",
        "opp": "@ DET",
        "decision": "W",
        "ip": "5.1",
        "er": 3,
        "h": 6,
        "bb": 2,
        "so": 5,
        "era": 5.06
      },
      {
        "date": "2026-06-28",
        "opp": "@ BOS",
        "decision": null,
        "ip": "5.0",
        "er": 0,
        "h": 1,
        "bb": 4,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-08-18",
        "opp": "@ BAL",
        "decision": null,
        "ip": "4.0",
        "er": 1,
        "h": 2,
        "bb": 2,
        "so": 4,
        "era": 2.25
      },
      {
        "date": "2026-08-23",
        "opp": "vs TOR",
        "decision": null,
        "ip": "4.0",
        "er": 1,
        "h": 2,
        "bb": 2,
        "so": 2,
        "era": 2.25
      },
      {
        "date": "2026-08-29",
        "opp": "vs BOS",
        "decision": "L",
        "ip": "4.2",
        "er": 2,
        "h": 3,
        "bb": 1,
        "so": 5,
        "era": 3.86
      },
      {
        "date": "2026-09-05",
        "opp": "@ SD",
        "decision": "W",
        "ip": "5.0",
        "er": 1,
        "h": 2,
        "bb": 3,
        "so": 7,
        "era": 1.8
      },
      {
        "date": "2026-09-11",
        "opp": "vs NYM",
        "decision": "W",
        "ip": "6.1",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 8,
        "era": 1.42
      },
      {
        "date": "2026-09-16",
        "opp": "@ MIN",
        "decision": null,
        "ip": "6.0",
        "er": 2,
        "h": 4,
        "bb": 3,
        "so": 5,
        "era": 3.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs TB",
        "decision": "W",
        "ip": "8.0",
        "er": 0,
        "h": 3,
        "bb": 0,
        "so": 8,
        "era": 0.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 51.5,
      "home": 48.5
    },
    "expected_score": {
      "away": 1.8,
      "home": 2.2
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "team_stats",
      "away_handedness": "R",
      "home_handedness": "L",
      "bullpen_game": false,
      "any_cold_sp": false,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.22,
        "bat": 0.25,
        "sit": 0.15
      },
      "away": {
        "sp_score": 59.5,
        "sp_detail": {
          "era": 3.9,
          "whip": 1.0,
          "k9": 6.1,
          "avg_ip": 6.0,
          "qs_rate": 50.0,
          "last3_era": 3.03,
          "last_start_era": 3.0,
          "last2_eras": [
            3.6,
            3.0
          ],
          "recent_avg_era": 3.0,
          "recent_bad_start": false,
          "trend": "stable",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 36,
          "fb_velo": 92.7,
          "pitch_arsenal": [
            "SI",
            "CH",
            "CT",
            "FB",
            "CB",
            "SL"
          ],
          "secondary_pitches": [
            "CH",
            "CB",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 31.7,
              "velo": 92.4
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 25.9,
              "velo": 78.3
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 21.7,
              "velo": 89.1
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 12.8,
              "velo": 92.7
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 4.2,
              "velo": 80.0
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 3.7,
              "velo": 85.1
            }
          ]
        },
        "bp_score": 74.2,
        "bp_detail": {
          "bullpen_era": 2.96,
          "recent_era": 2.0,
          "recent_appearances": 18,
          "recent_ip": 18.7,
          "team_era": 3.58,
          "bp_ip": 274.0,
          "bp_count": 7,
          "closer_era": 1.47,
          "closer_name": "Bryan Baker"
        },
        "bat_score": 28.4,
        "bat_detail": {
          "recent_avg": 0.242,
          "runs_per_g": 4.0,
          "hr_per_g": 1.1,
          "bb_per_g": 2.5,
          "season_ops": 0.72,
          "season_avg": 0.25,
          "season_slg": 0.4,
          "n_games": 10,
          "last5_rpg": 2.8,
          "last5_avg": 0.217,
          "bat_trend": "cold",
          "home_split": {
            "recent_avg": 0.227,
            "runs_per_g": 4.9,
            "hr_per_g": 1.0,
            "bb_per_g": 3.1,
            "season_ops": 0.72,
            "season_avg": 0.25,
            "season_slg": 0.4,
            "n_games": 10,
            "last5_rpg": 4.6,
            "last5_avg": 0.206,
            "bat_trend": "cold"
          },
          "away_split": {
            "recent_avg": 0.242,
            "runs_per_g": 4.0,
            "hr_per_g": 1.1,
            "bb_per_g": 2.5,
            "season_ops": 0.72,
            "season_avg": 0.25,
            "season_slg": 0.4,
            "n_games": 10,
            "last5_rpg": 2.8,
            "last5_avg": 0.217,
            "bat_trend": "cold"
          }
        },
        "sit_score": 62.8,
        "total": 55.5
      },
      "home": {
        "sp_score": 51.5,
        "sp_detail": {
          "era": 2.36,
          "whip": 0.97,
          "k9": 9.6,
          "avg_ip": 5.3,
          "qs_rate": 30.0,
          "last3_era": 1.8,
          "last_start_era": 0.0,
          "last2_eras": [
            3.0,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 33,
          "fb_velo": 94.3,
          "pitch_arsenal": [
            "FB",
            "SL",
            "CH",
            "SI"
          ],
          "secondary_pitches": [
            "SL",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 46.0,
              "velo": 94.3
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 25.5,
              "velo": 86.1
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 14.5,
              "velo": 85.2
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 12.4,
              "velo": 93.4
            }
          ]
        },
        "bp_score": 66.1,
        "bp_detail": {
          "bullpen_era": 3.01,
          "recent_era": 3.93,
          "recent_appearances": 19,
          "recent_ip": 18.3,
          "team_era": 2.97,
          "bp_ip": 443.0,
          "bp_count": 9,
          "closer_era": 2.54,
          "closer_name": "David Bednar"
        },
        "bat_score": 35.3,
        "bat_detail": {
          "recent_avg": 0.266,
          "runs_per_g": 5.2,
          "hr_per_g": 1.8,
          "bb_per_g": 2.9,
          "season_ops": 0.72,
          "season_avg": 0.25,
          "season_slg": 0.4,
          "n_games": 10,
          "last5_rpg": 1.8,
          "last5_avg": 0.165,
          "bat_trend": "cold",
          "home_split": {
            "recent_avg": 0.266,
            "runs_per_g": 5.2,
            "hr_per_g": 1.8,
            "bb_per_g": 2.9,
            "season_ops": 0.72,
            "season_avg": 0.25,
            "season_slg": 0.4,
            "n_games": 10,
            "last5_rpg": 1.8,
            "last5_avg": 0.165,
            "bat_trend": "cold"
          },
          "away_split": {
            "recent_avg": 0.24,
            "runs_per_g": 5.2,
            "hr_per_g": 1.3,
            "bb_per_g": 3.6,
            "season_ops": 0.72,
            "season_avg": 0.25,
            "season_slg": 0.4,
            "n_games": 10,
            "last5_rpg": 5.6,
            "last5_avg": 0.239,
            "bat_trend": "stable"
          }
        },
        "sit_score": 56.5,
        "total": 51.4
      }
    },
    "scores": {
      "away_offense": 28.4,
      "away_defense": 66.8,
      "home_offense": 35.3,
      "home_defense": 58.8
    },
    "actual_score": {
      "away": 0,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "New York Yankees",
    "model_winner": "Tampa Bay Rays",
    "model_correct": false,
    "notes": "Tampa Bay Rays IL: Edwin Uceta, Garrett Cleavinger, Gavin Lux, Jonathan Heasley, Manuel Rodríguez 외 다수 / New York Yankees IL: Aaron Judge, Fernando Cruz, Giancarlo Stanton, Kervin Castro, Ryan Weathers 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "⏭️ 패스 (라인업 미확정)",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 51.5% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "4:05 PM PT",
    "game_number": 2,
    "doubleheader": true,
    "game_pk": 823494,
    "away": "Tampa Bay Rays",
    "home": "New York Yankees",
    "away_standing": {
      "div_rank": 1,
      "div_name": "AL East",
      "wins": 96,
      "losses": 61,
      "games_back": "-"
    },
    "home_standing": {
      "div_rank": 2,
      "div_name": "AL East",
      "wins": 90,
      "losses": 67,
      "games_back": "6.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "4-3",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "10-1",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-4",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "2-1",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "5-1",
          "home": true
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "W",
          "score": "8-1",
          "home": false
        },
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "4-5",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "9-2",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "3-5",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "4-8",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -2
    },
    "away_pitcher": "Drew Rasmussen",
    "away_pitcher_id": 656876,
    "away_pitcher_stats": {
      "wins": 16,
      "losses": 5,
      "era": "2.67"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-01",
        "opp": "vs CWS",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 1,
        "so": 10,
        "era": 0.0
      },
      {
        "date": "2026-08-07",
        "opp": "@ SEA",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 2,
        "bb": 2,
        "so": 8,
        "era": 0.0
      },
      {
        "date": "2026-08-12",
        "opp": "@ OAK",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 4,
        "bb": 1,
        "so": 3,
        "era": 3.6
      },
      {
        "date": "2026-08-19",
        "opp": "vs TOR",
        "decision": "W",
        "ip": "5.0",
        "er": 5,
        "h": 7,
        "bb": 2,
        "so": 5,
        "era": 9.0
      },
      {
        "date": "2026-08-24",
        "opp": "@ DET",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 0,
        "so": 5,
        "era": 1.5
      },
      {
        "date": "2026-08-30",
        "opp": "vs SD",
        "decision": null,
        "ip": "6.0",
        "er": 2,
        "h": 4,
        "bb": 2,
        "so": 6,
        "era": 3.0
      },
      {
        "date": "2026-09-05",
        "opp": "@ TEX",
        "decision": null,
        "ip": "7.0",
        "er": 1,
        "h": 3,
        "bb": 0,
        "so": 7,
        "era": 1.29
      },
      {
        "date": "2026-09-11",
        "opp": "vs HOU",
        "decision": null,
        "ip": "7.0",
        "er": 1,
        "h": 8,
        "bb": 0,
        "so": 4,
        "era": 1.29
      },
      {
        "date": "2026-09-17",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "5.0",
        "er": 0,
        "h": 4,
        "bb": 2,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "@ NYY",
        "decision": "W",
        "ip": "6.2",
        "er": 1,
        "h": 1,
        "bb": 3,
        "so": 11,
        "era": 1.35
      }
    ],
    "home_pitcher": "Max Fried",
    "home_pitcher_id": 608331,
    "home_pitcher_stats": {
      "wins": 5,
      "losses": 5,
      "era": "2.80"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-07-22",
        "opp": "vs PIT",
        "decision": null,
        "ip": "5.0",
        "er": 0,
        "h": 1,
        "bb": 1,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-07-27",
        "opp": "@ CWS",
        "decision": null,
        "ip": "3.0",
        "er": 3,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 9.0
      },
      {
        "date": "2026-08-01",
        "opp": "@ CHC",
        "decision": null,
        "ip": "5.1",
        "er": 1,
        "h": 2,
        "bb": 1,
        "so": 7,
        "era": 1.69
      },
      {
        "date": "2026-08-07",
        "opp": "vs ATL",
        "decision": null,
        "ip": "6.1",
        "er": 0,
        "h": 3,
        "bb": 2,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-08-13",
        "opp": "vs SEA",
        "decision": "L",
        "ip": "5.0",
        "er": 1,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 1.8
      },
      {
        "date": "2026-08-29",
        "opp": "vs BOS",
        "decision": null,
        "ip": "3.2",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 2.45
      },
      {
        "date": "2026-09-04",
        "opp": "@ SD",
        "decision": null,
        "ip": "5.2",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 4,
        "era": 1.59
      },
      {
        "date": "2026-09-10",
        "opp": "vs COL",
        "decision": null,
        "ip": "3.1",
        "er": 1,
        "h": 1,
        "bb": 2,
        "so": 7,
        "era": 2.7
      },
      {
        "date": "2026-09-15",
        "opp": "@ MIN",
        "decision": "W",
        "ip": "7.0",
        "er": 1,
        "h": 6,
        "bb": 0,
        "so": 6,
        "era": 1.29
      },
      {
        "date": "2026-09-22",
        "opp": "vs TB",
        "decision": "L",
        "ip": "6.2",
        "er": 4,
        "h": 6,
        "bb": 2,
        "so": 4,
        "era": 5.4
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 61.4,
      "home": 38.6
    },
    "expected_score": {
      "away": 2.3,
      "home": 1.6
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "L",
      "bullpen_game": false,
      "any_cold_sp": false,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.22,
        "bat": 0.25,
        "sit": 0.15
      },
      "away": {
        "sp_score": 72.0,
        "sp_detail": {
          "era": 1.93,
          "whip": 0.87,
          "k9": 9.2,
          "avg_ip": 6.1,
          "qs_rate": 70.0,
          "last3_era": 1.42,
          "last_start_era": 1.35,
          "last2_eras": [
            0.0,
            1.35
          ],
          "recent_avg_era": 1.35,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 5,
          "rest_note": null,
          "age": 31,
          "fb_velo": 95.8,
          "pitch_arsenal": [
            "CT",
            "FB",
            "SI",
            "CH",
            "CB"
          ],
          "secondary_pitches": [
            "CH",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 28.9,
              "velo": 90.0
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 28.0,
              "velo": 95.8
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 23.8,
              "velo": 95.3
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 13.3,
              "velo": 89.6
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 3.5,
              "velo": 80.1
            }
          ]
        },
        "bp_score": 75.0,
        "bp_detail": {
          "bullpen_era": 2.96,
          "recent_era": 2.0,
          "recent_appearances": 18,
          "recent_ip": 18.7,
          "team_era": 3.58,
          "bp_ip": 274.0,
          "bp_count": 7,
          "closer_era": 1.47,
          "closer_name": "Bryan Baker"
        },
        "bat_score": 35.0,
        "bat_detail": {
          "recent_avg": 0.201,
          "runs_per_g": 4.5,
          "hr_per_g": 0.7,
          "bb_per_g": 3.0,
          "season_ops": 0.631,
          "season_slg": null,
          "season_avg": 0.201,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Yandy Díaz(0.530)",
            "Ryan Vilade(0.608)",
            "Junior Caminero(0.783)",
            "Jonny DeLuca(0.571)",
            "Chandler Simpson(0.722)",
            "Ben Williamson(0.584)",
            "Nick Fortes(0.767)",
            "Taylor Walls(0.645)",
            "Cedric Mullins(0.466)"
          ],
          "lineup_players": [
            {
              "id": 650490,
              "name": "Yandy Díaz",
              "ops": 0.53,
              "avg": 0.098
            },
            {
              "id": 668723,
              "name": "Ryan Vilade",
              "ops": 0.608,
              "avg": 0.13
            },
            {
              "id": 691406,
              "name": "Junior Caminero",
              "ops": 0.783,
              "avg": 0.2
            },
            {
              "id": 676356,
              "name": "Jonny DeLuca",
              "ops": 0.571,
              "avg": 0.172
            },
            {
              "id": 802415,
              "name": "Chandler Simpson",
              "ops": 0.722,
              "avg": 0.35
            },
            {
              "id": 810938,
              "name": "Ben Williamson",
              "ops": 0.584,
              "avg": 0.214
            },
            {
              "id": 663743,
              "name": "Nick Fortes",
              "ops": 0.767,
              "avg": 0.25
            },
            {
              "id": 670764,
              "name": "Taylor Walls",
              "ops": 0.645,
              "avg": 0.259
            },
            {
              "id": 656775,
              "name": "Cedric Mullins",
              "ops": 0.466,
              "avg": 0.136
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.6,
          "last5_avg": 0.199,
          "home_split": {
            "recent_avg": 0.227,
            "runs_per_g": 4.9,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.242,
            "runs_per_g": 4.0,
            "hr_per_g": 1.1,
            "n_games": 10
          }
        },
        "sit_score": 62.8,
        "total": 62.0
      },
      "home": {
        "sp_score": 49.2,
        "sp_detail": {
          "era": 2.29,
          "whip": 0.98,
          "k9": 9.5,
          "avg_ip": 5.1,
          "qs_rate": 20.0,
          "last3_era": 2.73,
          "last_start_era": 5.4,
          "last2_eras": [
            1.29,
            5.4
          ],
          "recent_avg_era": 5.4,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 32,
          "fb_velo": 94.8,
          "pitch_arsenal": [
            "CT",
            "SI",
            "FB",
            "CB",
            "SW",
            "CH"
          ],
          "secondary_pitches": [
            "CB",
            "SW",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 22.5,
              "velo": 91.8
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 20.9,
              "velo": 93.1
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 17.5,
              "velo": 94.8
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 13.6,
              "velo": 73.7
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 12.3,
              "velo": 79.8
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 12.1,
              "velo": 85.6
            }
          ]
        },
        "bp_score": 65.0,
        "bp_detail": {
          "bullpen_era": 3.01,
          "recent_era": 3.93,
          "recent_appearances": 19,
          "recent_ip": 18.3,
          "team_era": 2.97,
          "bp_ip": 443.0,
          "bp_count": 9,
          "closer_era": 2.54,
          "closer_name": "David Bednar"
        },
        "bat_score": 27.6,
        "bat_detail": {
          "recent_avg": 0.224,
          "runs_per_g": 4.3,
          "hr_per_g": 1.6,
          "bb_per_g": 3.0,
          "season_ops": 0.719,
          "season_slg": null,
          "season_avg": 0.224,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Ben Rice(0.999)",
            "Cody Bellinger(0.792)",
            "Heliot Ramos(0.710)",
            "Luis García Jr.(0.554)",
            "Spencer Jones(0.675)",
            "Jazz Chisholm Jr.(0.775)",
            "George Lombard Jr.(0.717)",
            "Austin Wells(0.774)",
            "Ryan McMahon(0.475)"
          ],
          "lineup_players": [
            {
              "id": 700250,
              "name": "Ben Rice",
              "ops": 0.999,
              "avg": 0.281
            },
            {
              "id": 641355,
              "name": "Cody Bellinger",
              "ops": 0.792,
              "avg": 0.268
            },
            {
              "id": 671218,
              "name": "Heliot Ramos",
              "ops": 0.71,
              "avg": 0.188
            },
            {
              "id": 671277,
              "name": "Luis García Jr.",
              "ops": 0.554,
              "avg": 0.139
            },
            {
              "id": 682987,
              "name": "Spencer Jones",
              "ops": 0.675,
              "avg": 0.189
            },
            {
              "id": 665862,
              "name": "Jazz Chisholm Jr.",
              "ops": 0.775,
              "avg": 0.323
            },
            {
              "id": 806146,
              "name": "George Lombard Jr.",
              "ops": 0.717,
              "avg": 0.27
            },
            {
              "id": 669224,
              "name": "Austin Wells",
              "ops": 0.774,
              "avg": 0.222
            },
            {
              "id": 641857,
              "name": "Ryan McMahon",
              "ops": 0.475,
              "avg": 0.138
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.4,
          "last5_avg": 0.187,
          "home_split": {
            "recent_avg": 0.266,
            "runs_per_g": 5.2,
            "hr_per_g": 1.8,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.24,
            "runs_per_g": 5.2,
            "hr_per_g": 1.3,
            "n_games": 10
          }
        },
        "sit_score": 56.5,
        "total": 48.4
      }
    },
    "scores": {
      "away_offense": 35.0,
      "away_defense": 73.5,
      "home_offense": 27.6,
      "home_defense": 57.1
    },
    "actual_score": {
      "away": 6,
      "home": 1
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Tampa Bay Rays",
    "model_winner": "Tampa Bay Rays",
    "model_correct": true,
    "notes": "Tampa Bay Rays IL: Edwin Uceta, Garrett Cleavinger, Gavin Lux, Jonathan Heasley, Manuel Rodríguez 외 다수 / New York Yankees IL: Aaron Judge, Fernando Cruz, Giancarlo Stanton, Kervin Castro, Ryan Weathers 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "3:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824222,
    "away": "Washington Nationals",
    "home": "Detroit Tigers",
    "away_standing": {
      "div_rank": 4,
      "div_name": "NL East",
      "wins": 75,
      "losses": 84,
      "games_back": "18.0"
    },
    "home_standing": {
      "div_rank": 4,
      "div_name": "AL Central",
      "wins": 74,
      "losses": 85,
      "games_back": "9.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "0-3",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "9-1",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "8-5",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "3-5",
          "home": false
        },
        {
          "date": "2026-09-21",
          "result": "L",
          "score": "2-9",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "1-3",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "11-8",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "1-3",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "1-8",
          "home": false
        },
        {
          "date": "2026-09-21",
          "result": "W",
          "score": "9-2",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": 1
    },
    "away_pitcher": "Riley Cornelio",
    "away_pitcher_id": 683000,
    "away_pitcher_stats": {
      "wins": 3,
      "losses": 2,
      "era": "3.89"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-20",
        "opp": "@ COL",
        "decision": "W",
        "ip": "1.2",
        "er": 0,
        "h": 0,
        "bb": 2,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-07-26",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "2.0",
        "er": 1,
        "h": 1,
        "bb": 2,
        "so": 0,
        "era": 4.5
      },
      {
        "date": "2026-07-30",
        "opp": "@ ATL",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 2,
        "era": 0.0
      },
      {
        "date": "2026-08-01",
        "opp": "@ ATL",
        "decision": null,
        "ip": "0.2",
        "er": 0,
        "h": 0,
        "bb": 3,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-09-02",
        "opp": "vs ATL",
        "decision": "L",
        "ip": "2.2",
        "er": 3,
        "h": 3,
        "bb": 1,
        "so": 3,
        "era": 10.12
      },
      {
        "date": "2026-09-08",
        "opp": "@ SD",
        "decision": null,
        "ip": "3.0",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-09-13",
        "opp": "vs LAA",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 1,
        "bb": 1,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-09-16",
        "opp": "vs PHI",
        "decision": null,
        "ip": "2.2",
        "er": 0,
        "h": 0,
        "bb": 2,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-09-19",
        "opp": "@ STL",
        "decision": "W",
        "ip": "1.2",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 2,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "@ DET",
        "decision": null,
        "ip": "2.2",
        "er": 0,
        "h": 1,
        "bb": 3,
        "so": 4,
        "era": 0.0
      }
    ],
    "home_pitcher": "Drew Anderson",
    "home_pitcher_id": 623454,
    "home_pitcher_stats": {
      "wins": 5,
      "losses": 7,
      "era": "3.67"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-02",
        "opp": "@ OAK",
        "decision": null,
        "ip": "1.1",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 2,
        "era": 0.0
      },
      {
        "date": "2026-08-05",
        "opp": "@ SEA",
        "decision": null,
        "ip": "3.2",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-11",
        "opp": "vs CLE",
        "decision": null,
        "ip": "4.0",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 4,
        "era": 2.25
      },
      {
        "date": "2026-08-16",
        "opp": "vs CWS",
        "decision": null,
        "ip": "5.0",
        "er": 3,
        "h": 4,
        "bb": 2,
        "so": 5,
        "era": 5.4
      },
      {
        "date": "2026-08-22",
        "opp": "@ KC",
        "decision": "L",
        "ip": "5.0",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 4,
        "era": 5.4
      },
      {
        "date": "2026-08-28",
        "opp": "vs LAD",
        "decision": null,
        "ip": "4.2",
        "er": 0,
        "h": 4,
        "bb": 5,
        "so": 5,
        "era": 0.0
      },
      {
        "date": "2026-09-02",
        "opp": "@ MIN",
        "decision": null,
        "ip": "5.2",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 5,
        "era": 1.59
      },
      {
        "date": "2026-09-08",
        "opp": "vs MIN",
        "decision": "L",
        "ip": "5.1",
        "er": 3,
        "h": 4,
        "bb": 3,
        "so": 4,
        "era": 5.06
      },
      {
        "date": "2026-09-15",
        "opp": "@ TOR",
        "decision": "W",
        "ip": "5.0",
        "er": 0,
        "h": 2,
        "bb": 3,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs WSH",
        "decision": "L",
        "ip": "6.0",
        "er": 3,
        "h": 5,
        "bb": 0,
        "so": 5,
        "era": 4.5
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 38.5,
      "home": 61.5
    },
    "expected_score": {
      "away": 2.0,
      "home": 2.3
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.33,
        "bp": 0.27,
        "bat": 0.33,
        "sit": 0.15
      },
      "away": {
        "sp_score": 47.5,
        "sp_detail": {
          "era": 1.8,
          "whip": 1.2,
          "k9": 9.5,
          "avg_ip": 2.0,
          "qs_rate": 0.0,
          "last3_era": 0.0,
          "last_start_era": 0.0,
          "last2_eras": [
            0.0,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 3,
          "rest_note": "short_rest",
          "age": 26,
          "fb_velo": 96.5,
          "pitch_arsenal": [
            "SL",
            "FB",
            "CT"
          ],
          "secondary_pitches": [
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 49.2,
              "velo": 84.8
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 37.6,
              "velo": 96.5
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 13.1,
              "velo": 90.1
            }
          ]
        },
        "bp_score": 25.0,
        "bp_detail": {
          "bullpen_era": 4.91,
          "recent_era": 4.19,
          "recent_appearances": 24,
          "recent_ip": 34.3,
          "team_era": 4.02,
          "bp_ip": 170.3,
          "bp_count": 9,
          "closer_era": 6.82,
          "closer_name": "Gus Varland"
        },
        "bat_score": 31.5,
        "bat_detail": {
          "recent_avg": 0.237,
          "runs_per_g": 4.7,
          "hr_per_g": 0.9,
          "bb_per_g": 3.0,
          "season_ops": 0.725,
          "season_slg": null,
          "season_avg": 0.237,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "James Wood(0.704)",
            "Daylen Lile(0.744)",
            "Dylan Crews(0.588)",
            "CJ Abrams(0.918)",
            "Andrés Chaparro(0.783)",
            "Jorbit Vivas(0.663)",
            "Yohandy Morales(0.868)",
            "Keibert Ruiz(0.744)",
            "Nasim Nuñez(0.509)"
          ],
          "lineup_players": [
            {
              "id": 695578,
              "name": "James Wood",
              "ops": 0.704,
              "avg": 0.216
            },
            {
              "id": 695734,
              "name": "Daylen Lile",
              "ops": 0.744,
              "avg": 0.229
            },
            {
              "id": 686611,
              "name": "Dylan Crews",
              "ops": 0.588,
              "avg": 0.211
            },
            {
              "id": 682928,
              "name": "CJ Abrams",
              "ops": 0.918,
              "avg": 0.308
            },
            {
              "id": 665953,
              "name": "Andrés Chaparro",
              "ops": 0.783,
              "avg": 0.241
            },
            {
              "id": 678391,
              "name": "Jorbit Vivas",
              "ops": 0.663,
              "avg": 0.258
            },
            {
              "id": 691002,
              "name": "Yohandy Morales",
              "ops": 0.868,
              "avg": 0.226
            },
            {
              "id": 660688,
              "name": "Keibert Ruiz",
              "ops": 0.744,
              "avg": 0.28
            },
            {
              "id": 683083,
              "name": "Nasim Nuñez",
              "ops": 0.509,
              "avg": 0.167
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 4.0,
          "last5_avg": 0.215,
          "home_split": {
            "recent_avg": 0.254,
            "runs_per_g": 4.4,
            "hr_per_g": 0.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.206,
            "runs_per_g": 4.2,
            "hr_per_g": 1.2,
            "n_games": 10
          }
        },
        "sit_score": 46.1,
        "total": 39.7
      },
      "home": {
        "sp_score": 49.0,
        "sp_detail": {
          "era": 2.76,
          "whip": 1.2,
          "k9": 7.5,
          "avg_ip": 4.6,
          "qs_rate": 10.0,
          "last3_era": 2.36,
          "last_start_era": 4.5,
          "last2_eras": [
            0.0,
            4.5
          ],
          "recent_avg_era": 4.5,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 32,
          "fb_velo": 95.3,
          "pitch_arsenal": [
            "FB",
            "CH",
            "CB",
            "SL"
          ],
          "secondary_pitches": [
            "CH",
            "CB",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 40.6,
              "velo": 95.3
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 31.1,
              "velo": 89.8
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 22.0,
              "velo": 80.1
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 4.8,
              "velo": 88.1
            }
          ]
        },
        "bp_score": 64.7,
        "bp_detail": {
          "bullpen_era": 3.31,
          "recent_era": 2.91,
          "recent_appearances": 24,
          "recent_ip": 34.0,
          "team_era": 3.57,
          "bp_ip": 377.7,
          "bp_count": 10,
          "closer_era": 3.27,
          "closer_name": "Kenley Jansen"
        },
        "bat_score": 30.299999999999997,
        "bat_detail": {
          "recent_avg": 0.273,
          "runs_per_g": 4.3,
          "hr_per_g": 0.8,
          "bb_per_g": 3.0,
          "season_ops": 0.776,
          "season_slg": null,
          "season_avg": 0.273,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Gleyber Torres(0.612)",
            "Hao-Yu Lee(0.807)",
            "Kevin McGonigle(0.889)",
            "Eduardo Valencia(1.018)",
            "Ben Malgeri(0.976)",
            "Max Clark(0.814)",
            "Spencer Torkelson(0.948)",
            "Javier Báez(0.556)",
            "John Peck(0.368)"
          ],
          "lineup_players": [
            {
              "id": 650402,
              "name": "Gleyber Torres",
              "ops": 0.612,
              "avg": 0.167
            },
            {
              "id": 701678,
              "name": "Hao-Yu Lee",
              "ops": 0.807,
              "avg": 0.302
            },
            {
              "id": 805808,
              "name": "Kevin McGonigle",
              "ops": 0.889,
              "avg": 0.366
            },
            {
              "id": 680664,
              "name": "Eduardo Valencia",
              "ops": 1.018,
              "avg": 0.433
            },
            {
              "id": 701162,
              "name": "Ben Malgeri",
              "ops": 0.976,
              "avg": 0.333
            },
            {
              "id": 703601,
              "name": "Max Clark",
              "ops": 0.814,
              "avg": 0.324
            },
            {
              "id": 679529,
              "name": "Spencer Torkelson",
              "ops": 0.948,
              "avg": 0.321
            },
            {
              "id": 595879,
              "name": "Javier Báez",
              "ops": 0.556,
              "avg": 0.136
            },
            {
              "id": 807726,
              "name": "John Peck",
              "ops": 0.368,
              "avg": 0.074
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.8,
          "last5_avg": 0.25,
          "home_split": {
            "recent_avg": 0.251,
            "runs_per_g": 5.2,
            "hr_per_g": 1.3,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.253,
            "runs_per_g": 4.2,
            "hr_per_g": 0.6,
            "n_games": 10
          }
        },
        "sit_score": 44.5,
        "total": 50.3
      }
    },
    "scores": {
      "away_offense": 31.5,
      "away_defense": 36.2,
      "home_offense": 30.299999999999997,
      "home_defense": 56.9
    },
    "actual_score": {
      "away": 3,
      "home": 1
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Washington Nationals",
    "model_winner": "Detroit Tigers",
    "model_correct": false,
    "notes": "Washington Nationals IL: Connelly Early, Drew Millas, Ken Waldichuk, Matt Waldron, Mitchell Parker 외 다수 / Detroit Tigers IL: Bailey Horn, Burch Smith, Colt Keith, Jack Flaherty, Justin Verlander 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "SP 점수 차이 1.5pt (≤5.0pt)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "3:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 823328,
    "away": "St. Louis Cardinals",
    "home": "Pittsburgh Pirates",
    "away_standing": {
      "div_rank": 4,
      "div_name": "NL Central",
      "wins": 76,
      "losses": 81,
      "games_back": "22.0"
    },
    "home_standing": {
      "div_rank": 3,
      "div_name": "NL Central",
      "wins": 80,
      "losses": 77,
      "games_back": "18.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "L",
          "score": "3-10",
          "home": true
        },
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "5-6",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "1-9",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "5-8",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "5-3",
          "home": true
        }
      ],
      "wins": 1,
      "losses": 4,
      "streak": 1
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "4-5",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "7-4",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "8-5",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "6-5",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "4-3",
          "home": true
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 4
    },
    "away_pitcher": "Andre Pallante",
    "away_pitcher_id": 669467,
    "away_pitcher_stats": {
      "wins": 12,
      "losses": 8,
      "era": "3.61"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-19",
        "opp": "@ OAK",
        "decision": null,
        "ip": "5.1",
        "er": 3,
        "h": 5,
        "bb": 2,
        "so": 3,
        "era": 5.06
      },
      {
        "date": "2026-07-25",
        "opp": "vs CIN",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 8,
        "era": 0.0
      },
      {
        "date": "2026-07-30",
        "opp": "vs CHC",
        "decision": null,
        "ip": "6.1",
        "er": 2,
        "h": 7,
        "bb": 2,
        "so": 3,
        "era": 2.84
      },
      {
        "date": "2026-08-05",
        "opp": "@ NYY",
        "decision": "W",
        "ip": "5.1",
        "er": 1,
        "h": 3,
        "bb": 3,
        "so": 4,
        "era": 1.69
      },
      {
        "date": "2026-08-11",
        "opp": "vs PHI",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 3,
        "so": 2,
        "era": 0.0
      },
      {
        "date": "2026-08-17",
        "opp": "@ CIN",
        "decision": null,
        "ip": "6.0",
        "er": 4,
        "h": 10,
        "bb": 1,
        "so": 5,
        "era": 6.0
      },
      {
        "date": "2026-09-04",
        "opp": "@ COL",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 2,
        "bb": 4,
        "so": 1,
        "era": 1.8
      },
      {
        "date": "2026-09-09",
        "opp": "@ SF",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 5,
        "bb": 0,
        "so": 4,
        "era": 1.8
      },
      {
        "date": "2026-09-15",
        "opp": "vs SF",
        "decision": "L",
        "ip": "5.0",
        "er": 6,
        "h": 8,
        "bb": 2,
        "so": 2,
        "era": 10.8
      },
      {
        "date": "2026-09-22",
        "opp": "@ PIT",
        "decision": "L",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 2,
        "so": 2,
        "era": 1.5
      }
    ],
    "home_pitcher": "Jared Jones",
    "home_pitcher_id": 683003,
    "home_pitcher_stats": {
      "wins": 5,
      "losses": 6,
      "era": "3.87"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-07-29",
        "opp": "vs OAK",
        "decision": "L",
        "ip": "6.0",
        "er": 3,
        "h": 4,
        "bb": 2,
        "so": 5,
        "era": 4.5
      },
      {
        "date": "2026-08-04",
        "opp": "@ MIL",
        "decision": "L",
        "ip": "4.0",
        "er": 3,
        "h": 8,
        "bb": 1,
        "so": 4,
        "era": 6.75
      },
      {
        "date": "2026-08-09",
        "opp": "vs NYM",
        "decision": "L",
        "ip": "3.0",
        "er": 8,
        "h": 6,
        "bb": 3,
        "so": 3,
        "era": 24.0
      },
      {
        "date": "2026-08-15",
        "opp": "vs BOS",
        "decision": "L",
        "ip": "5.0",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 7,
        "era": 1.8
      },
      {
        "date": "2026-08-22",
        "opp": "@ LAD",
        "decision": null,
        "ip": "4.0",
        "er": 2,
        "h": 3,
        "bb": 5,
        "so": 5,
        "era": 4.5
      },
      {
        "date": "2026-08-28",
        "opp": "@ STL",
        "decision": "L",
        "ip": "4.2",
        "er": 3,
        "h": 7,
        "bb": 1,
        "so": 6,
        "era": 5.79
      },
      {
        "date": "2026-09-04",
        "opp": "vs LAA",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 9,
        "era": 0.0
      },
      {
        "date": "2026-09-10",
        "opp": "@ CWS",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 1,
        "bb": 2,
        "so": 8,
        "era": 0.0
      },
      {
        "date": "2026-09-16",
        "opp": "vs MIL",
        "decision": null,
        "ip": "5.0",
        "er": 3,
        "h": 7,
        "bb": 1,
        "so": 9,
        "era": 5.4
      },
      {
        "date": "2026-09-22",
        "opp": "vs STL",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 9,
        "era": 0.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 38.5,
      "home": 61.5
    },
    "expected_score": {
      "away": 1.6,
      "home": 2.6
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 47.4,
        "sp_detail": {
          "era": 3.0,
          "whip": 1.21,
          "k9": 5.4,
          "avg_ip": 5.7,
          "qs_rate": 40.0,
          "last3_era": 4.33,
          "last_start_era": 1.5,
          "last2_eras": [
            10.8,
            1.5
          ],
          "recent_avg_era": 1.5,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 28,
          "fb_velo": 95.4,
          "pitch_arsenal": [
            "FB",
            "SL",
            "KC",
            "SI"
          ],
          "secondary_pitches": [
            "SL",
            "KC"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 33.9,
              "velo": 95.2
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 29.0,
              "velo": 87.8
            },
            {
              "code": "KC",
              "abbr": "KC",
              "pct": 17.3,
              "velo": 78.6
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 16.9,
              "velo": 95.4
            }
          ]
        },
        "bp_score": 50.8,
        "bp_detail": {
          "bullpen_era": 3.98,
          "recent_era": 3.43,
          "recent_appearances": 17,
          "recent_ip": 21.0,
          "team_era": 4.04,
          "bp_ip": 411.7,
          "bp_count": 9,
          "closer_era": 4.08,
          "closer_name": "Riley O'Brien"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.202,
          "runs_per_g": 3.2,
          "hr_per_g": 0.5,
          "bb_per_g": 3.0,
          "season_ops": 0.637,
          "season_slg": null,
          "season_avg": 0.202,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "JJ Wetherholt(0.572)",
            "Iván Herrera(0.744)",
            "Alec Burleson(0.715)",
            "Jordan Walker(0.522)",
            "Leo Bernal(0.800)",
            "Bryan Torres(0.508)",
            "Thomas Saggese(0.526)",
            "Nathan Church(0.754)",
            "Masyn Winn(0.595)"
          ],
          "lineup_players": [
            {
              "id": 802139,
              "name": "JJ Wetherholt",
              "ops": 0.572,
              "avg": 0.194
            },
            {
              "id": 671056,
              "name": "Iván Herrera",
              "ops": 0.744,
              "avg": 0.2
            },
            {
              "id": 676475,
              "name": "Alec Burleson",
              "ops": 0.715,
              "avg": 0.212
            },
            {
              "id": 691023,
              "name": "Jordan Walker",
              "ops": 0.522,
              "avg": 0.067
            },
            {
              "id": 699024,
              "name": "Leo Bernal",
              "ops": 0.8,
              "avg": 0.282
            },
            {
              "id": 663494,
              "name": "Bryan Torres",
              "ops": 0.508,
              "avg": 0.133
            },
            {
              "id": 695336,
              "name": "Thomas Saggese",
              "ops": 0.526,
              "avg": 0.167
            },
            {
              "id": 701675,
              "name": "Nathan Church",
              "ops": 0.754,
              "avg": 0.333
            },
            {
              "id": 691026,
              "name": "Masyn Winn",
              "ops": 0.595,
              "avg": 0.227
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 2.8,
          "last5_avg": 0.216,
          "home_split": {
            "recent_avg": 0.233,
            "runs_per_g": 4.0,
            "hr_per_g": 0.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.254,
            "runs_per_g": 4.8,
            "hr_per_g": 0.8,
            "n_games": 10
          }
        },
        "sit_score": 43.8,
        "total": 44.8
      },
      "home": {
        "sp_score": 55.0,
        "sp_detail": {
          "era": 4.01,
          "whip": 1.2,
          "k9": 11.3,
          "avg_ip": 5.2,
          "qs_rate": 40.0,
          "last3_era": 1.82,
          "last_start_era": 0.0,
          "last2_eras": [
            5.4,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 25,
          "fb_velo": 98.2,
          "pitch_arsenal": [
            "FB",
            "SL",
            "CH",
            "CB"
          ],
          "secondary_pitches": [
            "SL",
            "CH",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 46.8,
              "velo": 98.2
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 34.1,
              "velo": 89.8
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 11.0,
              "velo": 92.8
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 8.2,
              "velo": 85.7
            }
          ]
        },
        "bp_score": 61.9,
        "bp_detail": {
          "bullpen_era": 3.7,
          "recent_era": 2.21,
          "recent_appearances": 16,
          "recent_ip": 20.3,
          "team_era": 3.79,
          "bp_ip": 362.0,
          "bp_count": 8,
          "closer_era": 3.72,
          "closer_name": "Gregory Soto"
        },
        "bat_score": 36.099999999999994,
        "bat_detail": {
          "recent_avg": 0.274,
          "runs_per_g": 4.0,
          "hr_per_g": 1.3,
          "bb_per_g": 3.0,
          "season_ops": 0.769,
          "season_slg": null,
          "season_avg": 0.274,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Konnor Griffin(0.720)",
            "Brandon Lowe(1.008)",
            "Bryan Reynolds(0.676)",
            "Oneil Cruz(0.859)",
            "Rafael Flores(0.992)",
            "Ryan O'Hearn(0.703)",
            "Ronny Simon(0.720)",
            "Spencer Horwitz(0.708)",
            "Jacob Gonzalez(0.532)"
          ],
          "lineup_players": [
            {
              "id": 804606,
              "name": "Konnor Griffin",
              "ops": 0.72,
              "avg": 0.286
            },
            {
              "id": 664040,
              "name": "Brandon Lowe",
              "ops": 1.008,
              "avg": 0.324
            },
            {
              "id": 668804,
              "name": "Bryan Reynolds",
              "ops": 0.676,
              "avg": 0.211
            },
            {
              "id": 665833,
              "name": "Oneil Cruz",
              "ops": 0.859,
              "avg": 0.297
            },
            {
              "id": 804668,
              "name": "Rafael Flores",
              "ops": 0.992,
              "avg": 0.387
            },
            {
              "id": 656811,
              "name": "Ryan O'Hearn",
              "ops": 0.703,
              "avg": 0.219
            },
            {
              "id": 682927,
              "name": "Ronny Simon",
              "ops": 0.72,
              "avg": 0.24
            },
            {
              "id": 687462,
              "name": "Spencer Horwitz",
              "ops": 0.708,
              "avg": 0.3
            },
            {
              "id": 694378,
              "name": "Jacob Gonzalez",
              "ops": 0.532,
              "avg": 0.2
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 4.2,
          "last5_avg": 0.28,
          "home_split": {
            "recent_avg": 0.263,
            "runs_per_g": 3.5,
            "hr_per_g": 1.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.252,
            "runs_per_g": 3.6,
            "hr_per_g": 0.9,
            "n_games": 10
          }
        },
        "sit_score": 60.1,
        "total": 53.9
      }
    },
    "scores": {
      "away_offense": 25.0,
      "away_defense": 49.1,
      "home_offense": 36.099999999999994,
      "home_defense": 58.5
    },
    "actual_score": {
      "away": 0,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Pittsburgh Pirates",
    "model_winner": "Pittsburgh Pirates",
    "model_correct": true,
    "notes": "St. Louis Cardinals IL: Blaze Jordan, Everson Pereira, Hunter Dobbins, Joshua Báez, Max Rajcic / Pittsburgh Pirates IL: Braxton Ashcraft, Endy Rodríguez, Esmerlyn Valdez, Isaac Mattson, Mitch Keller",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "3:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 823412,
    "away": "Milwaukee Brewers",
    "home": "Philadelphia Phillies",
    "away_standing": {
      "div_rank": 1,
      "div_name": "NL Central",
      "wins": 98,
      "losses": 59,
      "games_back": "-"
    },
    "home_standing": {
      "div_rank": 2,
      "div_name": "NL East",
      "wins": 87,
      "losses": 70,
      "games_back": "5.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "5-4",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "4-7",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "6-5",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "1-0",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "3-0",
          "home": false
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 3
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "3-0",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "3-0",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "3-6",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "3-10",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "7-2",
          "home": false
        }
      ],
      "wins": 3,
      "losses": 2,
      "streak": 1
    },
    "away_pitcher": "Dustin May",
    "away_pitcher_id": 669160,
    "away_pitcher_stats": {
      "wins": 7,
      "losses": 9,
      "era": "4.67"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-29",
        "opp": "vs CHC",
        "decision": null,
        "ip": "5.0",
        "er": 0,
        "h": 1,
        "bb": 2,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-08-06",
        "opp": "vs PIT",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 3,
        "bb": 3,
        "so": 5,
        "era": 3.0
      },
      {
        "date": "2026-08-12",
        "opp": "@ SD",
        "decision": null,
        "ip": "7.0",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 6,
        "era": 1.29
      },
      {
        "date": "2026-08-19",
        "opp": "vs SEA",
        "decision": "L",
        "ip": "2.0",
        "er": 7,
        "h": 7,
        "bb": 2,
        "so": 1,
        "era": 31.5
      },
      {
        "date": "2026-08-26",
        "opp": "@ NYM",
        "decision": null,
        "ip": "0.1",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-30",
        "opp": "vs TEX",
        "decision": "L",
        "ip": "4.1",
        "er": 4,
        "h": 6,
        "bb": 3,
        "so": 4,
        "era": 8.31
      },
      {
        "date": "2026-09-05",
        "opp": "@ CIN",
        "decision": null,
        "ip": "3.2",
        "er": 2,
        "h": 5,
        "bb": 0,
        "so": 5,
        "era": 4.91
      },
      {
        "date": "2026-09-11",
        "opp": "vs CIN",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-18",
        "opp": "@ BAL",
        "decision": null,
        "ip": "2.1",
        "er": 3,
        "h": 6,
        "bb": 1,
        "so": 4,
        "era": 11.57
      },
      {
        "date": "2026-09-22",
        "opp": "@ PHI",
        "decision": null,
        "ip": "2.0",
        "er": 2,
        "h": 1,
        "bb": 2,
        "so": 2,
        "era": 9.0
      }
    ],
    "home_pitcher": "Zack Wheeler",
    "home_pitcher_id": 554430,
    "home_pitcher_stats": {
      "wins": 13,
      "losses": 5,
      "era": "3.06"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-02",
        "opp": "@ BAL",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-08-07",
        "opp": "vs TOR",
        "decision": "L",
        "ip": "5.1",
        "er": 4,
        "h": 7,
        "bb": 5,
        "so": 6,
        "era": 6.75
      },
      {
        "date": "2026-08-12",
        "opp": "@ STL",
        "decision": "L",
        "ip": "5.0",
        "er": 4,
        "h": 6,
        "bb": 1,
        "so": 5,
        "era": 7.2
      },
      {
        "date": "2026-08-18",
        "opp": "vs MIA",
        "decision": null,
        "ip": "5.0",
        "er": 2,
        "h": 2,
        "bb": 4,
        "so": 7,
        "era": 3.6
      },
      {
        "date": "2026-08-24",
        "opp": "@ SEA",
        "decision": "L",
        "ip": "4.0",
        "er": 8,
        "h": 9,
        "bb": 5,
        "so": 4,
        "era": 18.0
      },
      {
        "date": "2026-08-30",
        "opp": "@ LAA",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 3,
        "so": 6,
        "era": 1.5
      },
      {
        "date": "2026-09-05",
        "opp": "vs ATL",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 2,
        "so": 9,
        "era": 1.5
      },
      {
        "date": "2026-09-10",
        "opp": "vs HOU",
        "decision": null,
        "ip": "7.0",
        "er": 1,
        "h": 5,
        "bb": 2,
        "so": 9,
        "era": 1.29
      },
      {
        "date": "2026-09-16",
        "opp": "@ WSH",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 4,
        "bb": 1,
        "so": 5,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs MIL",
        "decision": null,
        "ip": "5.2",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 11,
        "era": 4.76
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 42.6,
      "home": 57.4
    },
    "expected_score": {
      "away": 2.2,
      "home": 1.9
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 30.0,
        "sp_detail": {
          "era": 4.89,
          "whip": 1.32,
          "k9": 8.4,
          "avg_ip": 3.9,
          "qs_rate": 30.0,
          "last3_era": 5.4,
          "last_start_era": 9.0,
          "last2_eras": [
            11.57,
            9.0
          ],
          "recent_avg_era": 9.0,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 4,
          "rest_note": null,
          "age": 29,
          "fb_velo": 96.9,
          "pitch_arsenal": [
            "FB",
            "CT",
            "SI",
            "SW",
            "CB",
            "CH"
          ],
          "secondary_pitches": [
            "SW",
            "CB",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 24.6,
              "velo": 96.9
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 23.8,
              "velo": 93.3
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 20.1,
              "velo": 96.6
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 17.9,
              "velo": 86.0
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 7.4,
              "velo": 83.5
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 6.2,
              "velo": 90.2
            }
          ]
        },
        "bp_score": 70.6,
        "bp_detail": {
          "bullpen_era": 3.11,
          "recent_era": 2.6,
          "recent_appearances": 16,
          "recent_ip": 17.3,
          "team_era": 3.37,
          "bp_ip": 388.0,
          "bp_count": 7,
          "closer_era": 2.72,
          "closer_name": "Trevor Megill"
        },
        "bat_score": 35.3,
        "bat_detail": {
          "recent_avg": 0.252,
          "runs_per_g": 4.8,
          "hr_per_g": 1.0,
          "bb_per_g": 3.0,
          "season_ops": 0.746,
          "season_slg": null,
          "season_avg": 0.252,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Garrett Mitchell(0.911)",
            "Jackson Chourio(0.639)",
            "Brice Turang(0.956)",
            "William Contreras(0.935)",
            "Jake Bauers(0.865)",
            "Christian Yelich(0.641)",
            "Luis Lara(0.560)",
            "Cooper Pratt(0.710)",
            "David Hamilton(0.498)"
          ],
          "lineup_players": [
            {
              "id": 669003,
              "name": "Garrett Mitchell",
              "ops": 0.911,
              "avg": 0.344
            },
            {
              "id": 694192,
              "name": "Jackson Chourio",
              "ops": 0.639,
              "avg": 0.132
            },
            {
              "id": 668930,
              "name": "Brice Turang",
              "ops": 0.956,
              "avg": 0.355
            },
            {
              "id": 661388,
              "name": "William Contreras",
              "ops": 0.935,
              "avg": 0.324
            },
            {
              "id": 641343,
              "name": "Jake Bauers",
              "ops": 0.865,
              "avg": 0.242
            },
            {
              "id": 592885,
              "name": "Christian Yelich",
              "ops": 0.641,
              "avg": 0.184
            },
            {
              "id": 800325,
              "name": "Luis Lara",
              "ops": 0.56,
              "avg": 0.211
            },
            {
              "id": 806198,
              "name": "Cooper Pratt",
              "ops": 0.71,
              "avg": 0.314
            },
            {
              "id": 666152,
              "name": "David Hamilton",
              "ops": 0.498,
              "avg": 0.158
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 3.6,
          "last5_avg": 0.2,
          "home_split": {
            "recent_avg": 0.313,
            "runs_per_g": 6.9,
            "hr_per_g": 1.6,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.224,
            "runs_per_g": 4.3,
            "hr_per_g": 0.7,
            "n_games": 10
          }
        },
        "sit_score": 59.7,
        "total": 45.9
      },
      "home": {
        "sp_score": 56.0,
        "sp_detail": {
          "era": 4.08,
          "whip": 1.32,
          "k9": 11.0,
          "avg_ip": 5.3,
          "qs_rate": 40.0,
          "last3_era": 1.71,
          "last_start_era": 4.76,
          "last2_eras": [
            0.0,
            4.76
          ],
          "recent_avg_era": 4.76,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 36,
          "fb_velo": 95.2,
          "pitch_arsenal": [
            "FB",
            "FS",
            "SI",
            "SW",
            "CT",
            "CB"
          ],
          "secondary_pitches": [
            "FS",
            "SW",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 35.8,
              "velo": 95.2
            },
            {
              "code": "FS",
              "abbr": "FS",
              "pct": 15.9,
              "velo": 87.4
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 15.6,
              "velo": 94.8
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 13.6,
              "velo": 81.3
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 10.6,
              "velo": 90.9
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 8.5,
              "velo": 80.0
            }
          ]
        },
        "bp_score": 67.5,
        "bp_detail": {
          "bullpen_era": 3.89,
          "recent_era": 2.0,
          "recent_appearances": 15,
          "recent_ip": 17.7,
          "team_era": 3.87,
          "bp_ip": 366.0,
          "bp_count": 9,
          "closer_era": 1.88,
          "closer_name": "Jhoan Duran"
        },
        "bat_score": 26.2,
        "bat_detail": {
          "recent_avg": 0.238,
          "runs_per_g": 4.0,
          "hr_per_g": 0.7,
          "bb_per_g": 3.0,
          "season_ops": 0.712,
          "season_slg": null,
          "season_avg": 0.238,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Kyle Schwarber(0.720)",
            "Trea Turner(0.688)",
            "Bryce Harper(0.795)",
            "Luis Arraez(0.705)",
            "Alec Bohm(0.849)",
            "Bryson Stott(0.720)",
            "Brandon Marsh(0.627)",
            "J.T. Realmuto(0.579)",
            "Justin Crawford(0.729)"
          ],
          "lineup_players": [
            {
              "id": 656941,
              "name": "Kyle Schwarber",
              "ops": 0.72,
              "avg": 0.125
            },
            {
              "id": 607208,
              "name": "Trea Turner",
              "ops": 0.688,
              "avg": 0.265
            },
            {
              "id": 547180,
              "name": "Bryce Harper",
              "ops": 0.795,
              "avg": 0.243
            },
            {
              "id": 650333,
              "name": "Luis Arraez",
              "ops": 0.705,
              "avg": 0.229
            },
            {
              "id": 664761,
              "name": "Alec Bohm",
              "ops": 0.849,
              "avg": 0.429
            },
            {
              "id": 681082,
              "name": "Bryson Stott",
              "ops": 0.72,
              "avg": 0.265
            },
            {
              "id": 669016,
              "name": "Brandon Marsh",
              "ops": 0.627,
              "avg": 0.167
            },
            {
              "id": 592663,
              "name": "J.T. Realmuto",
              "ops": 0.579,
              "avg": 0.156
            },
            {
              "id": 702222,
              "name": "Justin Crawford",
              "ops": 0.729,
              "avg": 0.259
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 4.0,
          "last5_avg": 0.222,
          "home_split": {
            "recent_avg": 0.231,
            "runs_per_g": 4.1,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.221,
            "runs_per_g": 3.8,
            "hr_per_g": 0.6,
            "n_games": 10
          }
        },
        "sit_score": 58.8,
        "total": 52.3
      }
    },
    "scores": {
      "away_offense": 35.3,
      "away_defense": 50.3,
      "home_offense": 26.2,
      "home_defense": 61.8
    },
    "actual_score": {
      "away": 4,
      "home": 6
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Philadelphia Phillies",
    "model_winner": "Philadelphia Phillies",
    "model_correct": true,
    "notes": "Milwaukee Brewers IL: Angel Zerpa, Brandon Woodruff, Brian Fitzpatrick, Bryse Wilson, Grant Anderson 외 다수 / Philadelphia Phillies IL: Adolis García, Brad Keller, Caleb Kilian, Felix Reyes, Jesús Luzardo 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "⏭️ 패스 (SP↔BAT충돌+Kalshi없음 — 시장 미검증, 신뢰불가)",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": true,
    "sp_bat_conflict_detail": {
      "sp_favors": "Philadelphia Phillies",
      "bat_favors": "Milwaukee Brewers",
      "sp_gap": -26.0,
      "bat_gap": 9.1
    },
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 57.4% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "3:45 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824709,
    "away": "Cleveland Guardians",
    "home": "Boston Red Sox",
    "away_standing": {
      "div_rank": 1,
      "div_name": "AL Central",
      "wins": 82,
      "losses": 75,
      "games_back": "-"
    },
    "home_standing": {
      "div_rank": 3,
      "div_name": "AL East",
      "wins": 84,
      "losses": 73,
      "games_back": "12.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "W",
          "score": "7-6",
          "home": true
        },
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "6-3",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "5-3",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "12-6",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "1-0",
          "home": true
        }
      ],
      "wins": 5,
      "losses": 0,
      "streak": 5
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "3-7",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "4-3",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "4-2",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "1-2",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "1-5",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -2
    },
    "away_pitcher": "Parker Messick",
    "away_pitcher_id": 800048,
    "away_pitcher_stats": {
      "wins": 12,
      "losses": 9,
      "era": "2.55"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-01",
        "opp": "vs OAK",
        "decision": null,
        "ip": "4.1",
        "er": 1,
        "h": 6,
        "bb": 1,
        "so": 6,
        "era": 2.08
      },
      {
        "date": "2026-08-07",
        "opp": "@ CWS",
        "decision": "W",
        "ip": "7.0",
        "er": 2,
        "h": 4,
        "bb": 1,
        "so": 8,
        "era": 2.57
      },
      {
        "date": "2026-08-13",
        "opp": "@ DET",
        "decision": "L",
        "ip": "5.2",
        "er": 2,
        "h": 3,
        "bb": 2,
        "so": 6,
        "era": 3.18
      },
      {
        "date": "2026-08-19",
        "opp": "vs SF",
        "decision": "L",
        "ip": "6.2",
        "er": 1,
        "h": 6,
        "bb": 0,
        "so": 10,
        "era": 1.35
      },
      {
        "date": "2026-08-24",
        "opp": "@ LAA",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 1.5
      },
      {
        "date": "2026-08-30",
        "opp": "vs KC",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 5,
        "bb": 2,
        "so": 5,
        "era": 1.5
      },
      {
        "date": "2026-09-05",
        "opp": "vs DET",
        "decision": "L",
        "ip": "6.0",
        "er": 3,
        "h": 7,
        "bb": 2,
        "so": 12,
        "era": 4.5
      },
      {
        "date": "2026-09-11",
        "opp": "@ MIN",
        "decision": null,
        "ip": "5.1",
        "er": 1,
        "h": 3,
        "bb": 2,
        "so": 5,
        "era": 1.69
      },
      {
        "date": "2026-09-16",
        "opp": "vs CWS",
        "decision": "W",
        "ip": "6.0",
        "er": 3,
        "h": 7,
        "bb": 0,
        "so": 8,
        "era": 4.5
      },
      {
        "date": "2026-09-22",
        "opp": "@ BOS",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 5,
        "bb": 1,
        "so": 5,
        "era": 1.8
      }
    ],
    "home_pitcher": "Payton Tolle",
    "home_pitcher_id": 801139,
    "home_pitcher_stats": {
      "wins": 9,
      "losses": 6,
      "era": "3.03"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-07-27",
        "opp": "@ OAK",
        "decision": "W",
        "ip": "5.1",
        "er": 2,
        "h": 5,
        "bb": 1,
        "so": 7,
        "era": 3.38
      },
      {
        "date": "2026-08-01",
        "opp": "@ LAD",
        "decision": null,
        "ip": "6.0",
        "er": 2,
        "h": 9,
        "bb": 2,
        "so": 7,
        "era": 3.0
      },
      {
        "date": "2026-08-07",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 2,
        "bb": 0,
        "so": 14,
        "era": 1.5
      },
      {
        "date": "2026-08-13",
        "opp": "@ TOR",
        "decision": "W",
        "ip": "8.0",
        "er": 0,
        "h": 2,
        "bb": 2,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-08-19",
        "opp": "vs OAK",
        "decision": null,
        "ip": "5.0",
        "er": 3,
        "h": 6,
        "bb": 2,
        "so": 6,
        "era": 5.4
      },
      {
        "date": "2026-08-25",
        "opp": "@ MIA",
        "decision": null,
        "ip": "5.1",
        "er": 2,
        "h": 4,
        "bb": 2,
        "so": 7,
        "era": 3.38
      },
      {
        "date": "2026-08-31",
        "opp": "vs SEA",
        "decision": null,
        "ip": "5.2",
        "er": 4,
        "h": 6,
        "bb": 2,
        "so": 6,
        "era": 6.35
      },
      {
        "date": "2026-09-06",
        "opp": "@ BAL",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 0,
        "so": 12,
        "era": 1.5
      },
      {
        "date": "2026-09-13",
        "opp": "vs KC",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 1,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs CLE",
        "decision": null,
        "ip": "5.2",
        "er": 2,
        "h": 8,
        "bb": 2,
        "so": 5,
        "era": 3.18
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 50.7,
      "home": 49.3
    },
    "expected_score": {
      "away": 2.5,
      "home": 1.5
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "L",
      "home_handedness": "L",
      "bullpen_game": false,
      "any_cold_sp": false,
      "eff_weights": {
        "sp": 0.35,
        "bp": 0.22,
        "bat": 0.28,
        "sit": 0.15
      },
      "away": {
        "sp_score": 72.0,
        "sp_detail": {
          "era": 2.48,
          "whip": 1.05,
          "k9": 10.7,
          "avg_ip": 5.8,
          "qs_rate": 60.0,
          "last3_era": 2.86,
          "last_start_era": 1.8,
          "last2_eras": [
            4.5,
            1.8
          ],
          "recent_avg_era": 1.8,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 25,
          "fb_velo": 94.4,
          "pitch_arsenal": [
            "FB",
            "CH",
            "SI",
            "CB",
            "SL",
            "CT"
          ],
          "secondary_pitches": [
            "CH",
            "CB",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 41.6,
              "velo": 94.4
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 23.5,
              "velo": 85.9
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 15.0,
              "velo": 92.8
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 8.6,
              "velo": 79.3
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 6.4,
              "velo": 87.0
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 4.8,
              "velo": 90.6
            }
          ]
        },
        "bp_score": 73.5,
        "bp_detail": {
          "bullpen_era": 3.21,
          "recent_era": 2.51,
          "recent_appearances": 15,
          "recent_ip": 14.3,
          "team_era": 3.59,
          "bp_ip": 381.3,
          "bp_count": 8,
          "closer_era": 1.99,
          "closer_name": "Cade Smith"
        },
        "bat_score": 42.5,
        "bat_detail": {
          "recent_avg": 0.267,
          "runs_per_g": 4.9,
          "hr_per_g": 1.3,
          "bb_per_g": 3.0,
          "season_ops": 0.762,
          "season_slg": null,
          "season_avg": 0.267,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Steven Kwan(0.828)",
            "José Ramírez(0.681)",
            "Chase DeLauter(0.823)",
            "Jo Adell(0.747)",
            "Angel Martínez(0.964)",
            "David Fry(0.621)",
            "Travis Bazzana(0.806)",
            "Austin Hedges(0.632)",
            "Brayan Rocchio(0.753)"
          ],
          "lineup_players": [
            {
              "id": 680757,
              "name": "Steven Kwan",
              "ops": 0.828,
              "avg": 0.353
            },
            {
              "id": 608070,
              "name": "José Ramírez",
              "ops": 0.681,
              "avg": 0.25
            },
            {
              "id": 800050,
              "name": "Chase DeLauter",
              "ops": 0.823,
              "avg": 0.257
            },
            {
              "id": 666176,
              "name": "Jo Adell",
              "ops": 0.747,
              "avg": 0.289
            },
            {
              "id": 682657,
              "name": "Angel Martínez",
              "ops": 0.964,
              "avg": 0.375
            },
            {
              "id": 681807,
              "name": "David Fry",
              "ops": 0.621,
              "avg": 0.182
            },
            {
              "id": 683953,
              "name": "Travis Bazzana",
              "ops": 0.806,
              "avg": 0.229
            },
            {
              "id": 595978,
              "name": "Austin Hedges",
              "ops": 0.632,
              "avg": 0.167
            },
            {
              "id": 677587,
              "name": "Brayan Rocchio",
              "ops": 0.753,
              "avg": 0.3
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 4.2,
          "last5_avg": 0.304,
          "home_split": {
            "recent_avg": 0.244,
            "runs_per_g": 4.8,
            "hr_per_g": 1.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.282,
            "runs_per_g": 5.0,
            "hr_per_g": 0.8,
            "n_games": 10
          }
        },
        "sit_score": 67.8,
        "total": 63.4
      },
      "home": {
        "sp_score": 69.6,
        "sp_detail": {
          "era": 2.59,
          "whip": 1.08,
          "k9": 11.4,
          "avg_ip": 5.9,
          "qs_rate": 50.0,
          "last3_era": 2.83,
          "last_start_era": 3.18,
          "last2_eras": [
            0.0,
            3.18
          ],
          "recent_avg_era": 3.18,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 9,
          "rest_note": "extra_rest",
          "age": 23,
          "fb_velo": 96.1,
          "pitch_arsenal": [
            "FB",
            "SI",
            "CT",
            "CB"
          ],
          "secondary_pitches": [
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 44.6,
              "velo": 96.1
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 24.1,
              "velo": 94.8
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 19.2,
              "velo": 88.6
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 9.3,
              "velo": 82.9
            }
          ]
        },
        "bp_score": 72.5,
        "bp_detail": {
          "bullpen_era": 2.74,
          "recent_era": 3.46,
          "recent_appearances": 12,
          "recent_ip": 13.0,
          "team_era": 3.27,
          "bp_ip": 364.7,
          "bp_count": 8,
          "closer_era": 1.95,
          "closer_name": "Aroldis Chapman"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.184,
          "runs_per_g": 2.6,
          "hr_per_g": 0.8,
          "bb_per_g": 3.0,
          "season_ops": 0.61,
          "season_slg": null,
          "season_avg": 0.184,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Roman Anthony(0.664)",
            "Nick Sogard(0.671)",
            "Jahmai Jones(0.566)",
            "Wilyer Abreu(0.667)",
            "Adley Rutschman(0.716)",
            "Caleb Durbin(0.857)",
            "Andruw Monasterio(0.399)",
            "Isiah Kiner-Falefa(0.460)",
            "Nate Eaton(0.487)"
          ],
          "lineup_players": [
            {
              "id": 701350,
              "name": "Roman Anthony",
              "ops": 0.664,
              "avg": 0.231
            },
            {
              "id": 686765,
              "name": "Nick Sogard",
              "ops": 0.671,
              "avg": 0.206
            },
            {
              "id": 663330,
              "name": "Jahmai Jones",
              "ops": 0.566,
              "avg": 0.136
            },
            {
              "id": 677800,
              "name": "Wilyer Abreu",
              "ops": 0.667,
              "avg": 0.2
            },
            {
              "id": 668939,
              "name": "Adley Rutschman",
              "ops": 0.716,
              "avg": 0.257
            },
            {
              "id": 702332,
              "name": "Caleb Durbin",
              "ops": 0.857,
              "avg": 0.346
            },
            {
              "id": 655316,
              "name": "Andruw Monasterio",
              "ops": 0.399,
              "avg": 0.042
            },
            {
              "id": 643396,
              "name": "Isiah Kiner-Falefa",
              "ops": 0.46,
              "avg": 0.08
            },
            {
              "id": 681987,
              "name": "Nate Eaton",
              "ops": 0.487,
              "avg": 0.158
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 1.6,
          "last5_avg": 0.239,
          "home_split": {
            "recent_avg": 0.258,
            "runs_per_g": 3.2,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.189,
            "runs_per_g": 3.0,
            "hr_per_g": 1.2,
            "n_games": 10
          }
        },
        "sit_score": 45.5,
        "total": 54.1
      }
    },
    "scores": {
      "away_offense": 42.5,
      "away_defense": 72.8,
      "home_offense": 25.0,
      "home_defense": 71.0
    },
    "actual_score": {
      "away": 3,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Cleveland Guardians",
    "model_winner": "Cleveland Guardians",
    "model_correct": true,
    "notes": "Cleveland Guardians IL: Colin Holderman, Rhys Hoskins / Boston Red Sox IL: Anthony Seigler, Curtis Mead, Eduardo Rivera, Eli White, Garrett Crochet 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "SP 점수 차이 2.4pt (≤5.0pt) / 최고 확률 50.7% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "4:15 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824867,
    "away": "Cincinnati Reds",
    "home": "Atlanta Braves",
    "away_standing": {
      "div_rank": 5,
      "div_name": "NL Central",
      "wins": 73,
      "losses": 84,
      "games_back": "25.0"
    },
    "home_standing": {
      "div_rank": 1,
      "div_name": "NL East",
      "wins": 92,
      "losses": 65,
      "games_back": "-"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "6-2",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "2-8",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "6-4",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "2-5",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "1-9",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "W",
          "score": "6-3",
          "home": false
        },
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "4-8",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "6-2",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "6-3",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "4-2",
          "home": false
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 3
    },
    "away_pitcher": "Brandon Williamson",
    "away_pitcher_id": 682227,
    "away_pitcher_stats": {
      "wins": 5,
      "losses": 4,
      "era": "4.89"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-04-06",
        "opp": "@ MIA",
        "decision": "W",
        "ip": "6.2",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-04-11",
        "opp": "vs LAA",
        "decision": null,
        "ip": "4.0",
        "er": 3,
        "h": 3,
        "bb": 6,
        "so": 3,
        "era": 6.75
      },
      {
        "date": "2026-04-17",
        "opp": "@ MIN",
        "decision": "W",
        "ip": "5.1",
        "er": 1,
        "h": 3,
        "bb": 4,
        "so": 2,
        "era": 1.69
      },
      {
        "date": "2026-04-22",
        "opp": "@ TB",
        "decision": "L",
        "ip": "4.1",
        "er": 5,
        "h": 7,
        "bb": 3,
        "so": 3,
        "era": 10.38
      },
      {
        "date": "2026-04-29",
        "opp": "vs COL",
        "decision": "L",
        "ip": "3.0",
        "er": 4,
        "h": 4,
        "bb": 4,
        "so": 4,
        "era": 12.0
      },
      {
        "date": "2026-09-02",
        "opp": "vs SD",
        "decision": "W",
        "ip": "5.1",
        "er": 3,
        "h": 7,
        "bb": 2,
        "so": 5,
        "era": 5.06
      },
      {
        "date": "2026-09-07",
        "opp": "@ LAD",
        "decision": "L",
        "ip": "4.1",
        "er": 3,
        "h": 3,
        "bb": 5,
        "so": 4,
        "era": 6.23
      },
      {
        "date": "2026-09-13",
        "opp": "@ MIL",
        "decision": "W",
        "ip": "4.0",
        "er": 1,
        "h": 2,
        "bb": 0,
        "so": 2,
        "era": 2.25
      },
      {
        "date": "2026-09-17",
        "opp": "vs LAD",
        "decision": null,
        "ip": "2.0",
        "er": 1,
        "h": 2,
        "bb": 0,
        "so": 1,
        "era": 4.5
      },
      {
        "date": "2026-09-22",
        "opp": "@ ATL",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 3,
        "bb": 0,
        "so": 2,
        "era": 0.0
      }
    ],
    "home_pitcher": "JR Ritchie",
    "home_pitcher_id": 702275,
    "home_pitcher_stats": {
      "wins": 1,
      "losses": 4,
      "era": "4.79"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-06-17",
        "opp": "vs SF",
        "decision": "L",
        "ip": "5.0",
        "er": 5,
        "h": 5,
        "bb": 3,
        "so": 4,
        "era": 9.0
      },
      {
        "date": "2026-06-23",
        "opp": "@ SD",
        "decision": null,
        "ip": "5.0",
        "er": 4,
        "h": 5,
        "bb": 4,
        "so": 7,
        "era": 7.2
      },
      {
        "date": "2026-07-04",
        "opp": "vs NYM",
        "decision": null,
        "ip": "3.0",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-07-09",
        "opp": "@ PIT",
        "decision": null,
        "ip": "1.1",
        "er": 1,
        "h": 2,
        "bb": 2,
        "so": 1,
        "era": 6.75
      },
      {
        "date": "2026-07-12",
        "opp": "@ STL",
        "decision": null,
        "ip": "4.1",
        "er": 1,
        "h": 1,
        "bb": 2,
        "so": 2,
        "era": 2.08
      },
      {
        "date": "2026-07-18",
        "opp": "vs TEX",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-07-21",
        "opp": "vs SD",
        "decision": null,
        "ip": "3.2",
        "er": 3,
        "h": 3,
        "bb": 3,
        "so": 4,
        "era": 7.36
      },
      {
        "date": "2026-08-02",
        "opp": "vs WSH",
        "decision": null,
        "ip": "4.0",
        "er": 2,
        "h": 4,
        "bb": 3,
        "so": 4,
        "era": 4.5
      },
      {
        "date": "2026-09-16",
        "opp": "@ CHC",
        "decision": "L",
        "ip": "4.2",
        "er": 6,
        "h": 7,
        "bb": 2,
        "so": 5,
        "era": 11.57
      },
      {
        "date": "2026-09-22",
        "opp": "vs CIN",
        "decision": "L",
        "ip": "5.0",
        "er": 1,
        "h": 4,
        "bb": 3,
        "so": 8,
        "era": 1.8
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 49.1,
      "home": 50.9
    },
    "expected_score": {
      "away": 1.6,
      "home": 2.3
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "L",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 41.5,
        "sp_detail": {
          "era": 4.2,
          "whip": 1.38,
          "k9": 6.0,
          "avg_ip": 4.5,
          "qs_rate": 20.0,
          "last3_era": 3.32,
          "last_start_era": 0.0,
          "last2_eras": [
            4.5,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "stable",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 5,
          "rest_note": null,
          "age": 28,
          "fb_velo": 93.1,
          "pitch_arsenal": [
            "CT",
            "CH",
            "FB",
            "SW",
            "CB",
            "SI"
          ],
          "secondary_pitches": [
            "CH",
            "SW",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 33.2,
              "velo": 89.5
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 21.4,
              "velo": 85.3
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 17.3,
              "velo": 92.8
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 11.0,
              "velo": 81.4
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 8.7,
              "velo": 75.2
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 8.4,
              "velo": 93.1
            }
          ]
        },
        "bp_score": 56.5,
        "bp_detail": {
          "bullpen_era": 3.99,
          "recent_era": 2.7,
          "recent_appearances": 17,
          "recent_ip": 20.0,
          "team_era": 4.83,
          "bp_ip": 320.7,
          "bp_count": 9,
          "closer_era": 3.46,
          "closer_name": "Emilio Pagán"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.192,
          "runs_per_g": 2.7,
          "hr_per_g": 1.0,
          "bb_per_g": 3.0,
          "season_ops": 0.638,
          "season_slg": null,
          "season_avg": 0.192,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Carlos Jorge(0.376)",
            "Elly De La Cruz(1.027)",
            "Sal Stewart(0.703)",
            "Tyler Stephenson(0.658)",
            "JJ Bleday(0.733)",
            "Eugenio Suárez(0.781)",
            "Dane Myers(0.637)",
            "Héctor Rodríguez(0.400)",
            "Matt McLain(0.428)"
          ],
          "lineup_players": [
            {
              "id": 699115,
              "name": "Carlos Jorge",
              "ops": 0.376,
              "avg": 0.095
            },
            {
              "id": 682829,
              "name": "Elly De La Cruz",
              "ops": 1.027,
              "avg": 0.355
            },
            {
              "id": 701398,
              "name": "Sal Stewart",
              "ops": 0.703,
              "avg": 0.25
            },
            {
              "id": 663886,
              "name": "Tyler Stephenson",
              "ops": 0.658,
              "avg": 0.219
            },
            {
              "id": 668709,
              "name": "JJ Bleday",
              "ops": 0.733,
              "avg": 0.265
            },
            {
              "id": 553993,
              "name": "Eugenio Suárez",
              "ops": 0.781,
              "avg": 0.226
            },
            {
              "id": 667472,
              "name": "Dane Myers",
              "ops": 0.637,
              "avg": 0.156
            },
            {
              "id": 699302,
              "name": "Héctor Rodríguez",
              "ops": 0.4,
              "avg": 0.059
            },
            {
              "id": 680574,
              "name": "Matt McLain",
              "ops": 0.428,
              "avg": 0.107
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 2.8,
          "last5_avg": 0.204,
          "home_split": {
            "recent_avg": 0.243,
            "runs_per_g": 4.2,
            "hr_per_g": 1.1,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.227,
            "runs_per_g": 3.6,
            "hr_per_g": 1.7,
            "n_games": 10
          }
        },
        "sit_score": 47.5,
        "total": 42.4
      },
      "home": {
        "sp_score": 30.0,
        "sp_detail": {
          "era": 5.59,
          "whip": 1.57,
          "k9": 9.7,
          "avg_ip": 3.7,
          "qs_rate": 0.0,
          "last3_era": 5.89,
          "last_start_era": 1.8,
          "last2_eras": [
            11.57,
            1.8
          ],
          "recent_avg_era": 1.8,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 23,
          "fb_velo": 94.7,
          "pitch_arsenal": [
            "CB",
            "SI",
            "FB",
            "CH",
            "CT",
            "SL"
          ],
          "secondary_pitches": [
            "CB",
            "CH",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 27.3,
              "velo": 83.0
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 21.3,
              "velo": 94.2
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 19.4,
              "velo": 94.7
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 17.5,
              "velo": 88.1
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 8.6,
              "velo": 89.2
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 6.0,
              "velo": 85.0
            }
          ]
        },
        "bp_score": 75.0,
        "bp_detail": {
          "bullpen_era": 2.69,
          "recent_era": 2.53,
          "recent_appearances": 11,
          "recent_ip": 10.7,
          "team_era": 3.14,
          "bp_ip": 408.3,
          "bp_count": 8,
          "closer_era": 2.64,
          "closer_name": "Raisel Iglesias"
        },
        "bat_score": 32.0,
        "bat_detail": {
          "recent_avg": 0.263,
          "runs_per_g": 4.5,
          "hr_per_g": 1.3,
          "bb_per_g": 3.0,
          "season_ops": 0.709,
          "season_slg": null,
          "season_avg": 0.263,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Ronald Acuña(0.972)",
            "Drake Baldwin(0.885)",
            "Matt Olson(0.847)",
            "Ozzie Albies(0.582)",
            "Michael Harris(0.905)",
            "Mauricio Dubón(0.837)",
            "Austin Riley(0.619)",
            "Sean Murphy(0.394)",
            "Ha-Seong Kim(0.339)"
          ],
          "lineup_players": [
            {
              "id": 660670,
              "name": "Ronald Acuña",
              "ops": 0.972,
              "avg": 0.342
            },
            {
              "id": 686948,
              "name": "Drake Baldwin",
              "ops": 0.885,
              "avg": 0.368
            },
            {
              "id": 621566,
              "name": "Matt Olson",
              "ops": 0.847,
              "avg": 0.333
            },
            {
              "id": 645277,
              "name": "Ozzie Albies",
              "ops": 0.582,
              "avg": 0.143
            },
            {
              "id": 671739,
              "name": "Michael Harris",
              "ops": 0.905,
              "avg": 0.395
            },
            {
              "id": 643289,
              "name": "Mauricio Dubón",
              "ops": 0.837,
              "avg": 0.389
            },
            {
              "id": 663586,
              "name": "Austin Riley",
              "ops": 0.619,
              "avg": 0.206
            },
            {
              "id": 669221,
              "name": "Sean Murphy",
              "ops": 0.394,
              "avg": 0.065
            },
            {
              "id": 673490,
              "name": "Ha-Seong Kim",
              "ops": 0.339,
              "avg": 0.125
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 3.2,
          "last5_avg": 0.253,
          "home_split": {
            "recent_avg": 0.253,
            "runs_per_g": 3.4,
            "hr_per_g": 0.8,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.231,
            "runs_per_g": 4.1,
            "hr_per_g": 1.4,
            "n_games": 10
          }
        },
        "sit_score": 60.7,
        "total": 43.2
      }
    },
    "scores": {
      "away_offense": 25.0,
      "away_defense": 49.0,
      "home_offense": 32.0,
      "home_defense": 59.2
    },
    "actual_score": {
      "away": 4,
      "home": 0
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Cincinnati Reds",
    "model_winner": "Atlanta Braves",
    "model_correct": false,
    "notes": "Cincinnati Reds IL: Blake Dunn, Chase Burns, Hunter Greene, Julian Garcia, Michael Toglia 외 다수 / Atlanta Braves IL: Bryce Elder, Joe Jiménez, Joey Wentz, Lane Thomas, Reynaldo López 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 50.9% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "4:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824061,
    "away": "Chicago White Sox",
    "home": "Kansas City Royals",
    "away_standing": {
      "div_rank": 2,
      "div_name": "AL Central",
      "wins": 81,
      "losses": 76,
      "games_back": "1.0"
    },
    "home_standing": {
      "div_rank": 5,
      "div_name": "AL Central",
      "wins": 67,
      "losses": 90,
      "games_back": "15.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "3-6",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "3-1",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "8-11",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "3-1",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "8-1",
          "home": true
        }
      ],
      "wins": 3,
      "losses": 2,
      "streak": 2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "5-2",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "2-6",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "5-8",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "5-6",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "3-4",
          "home": false
        }
      ],
      "wins": 1,
      "losses": 4,
      "streak": -4
    },
    "away_pitcher": "Anthony Kay",
    "away_pitcher_id": 641743,
    "away_pitcher_stats": {
      "wins": 9,
      "losses": 9,
      "era": "4.53"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-02",
        "opp": "@ TB",
        "decision": "W",
        "ip": "6.1",
        "er": 0,
        "h": 7,
        "bb": 1,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-08-08",
        "opp": "vs CLE",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 6,
        "bb": 1,
        "so": 4,
        "era": 3.0
      },
      {
        "date": "2026-08-15",
        "opp": "@ DET",
        "decision": null,
        "ip": "5.1",
        "er": 3,
        "h": 6,
        "bb": 1,
        "so": 4,
        "era": 5.06
      },
      {
        "date": "2026-08-20",
        "opp": "vs ATL",
        "decision": "L",
        "ip": "6.0",
        "er": 2,
        "h": 5,
        "bb": 1,
        "so": 6,
        "era": 3.0
      },
      {
        "date": "2026-08-25",
        "opp": "vs TEX",
        "decision": null,
        "ip": "4.2",
        "er": 4,
        "h": 6,
        "bb": 2,
        "so": 4,
        "era": 7.71
      },
      {
        "date": "2026-08-31",
        "opp": "@ HOU",
        "decision": "L",
        "ip": "4.2",
        "er": 5,
        "h": 6,
        "bb": 2,
        "so": 5,
        "era": 9.64
      },
      {
        "date": "2026-09-05",
        "opp": "vs MIN",
        "decision": "L",
        "ip": "4.1",
        "er": 5,
        "h": 6,
        "bb": 1,
        "so": 3,
        "era": 10.38
      },
      {
        "date": "2026-09-11",
        "opp": "@ STL",
        "decision": "L",
        "ip": "4.0",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 2,
        "era": 6.75
      },
      {
        "date": "2026-09-16",
        "opp": "@ CLE",
        "decision": null,
        "ip": "4.0",
        "er": 0,
        "h": 2,
        "bb": 2,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "@ KC",
        "decision": null,
        "ip": "4.0",
        "er": 4,
        "h": 7,
        "bb": 1,
        "so": 4,
        "era": 9.0
      }
    ],
    "home_pitcher": "Daniel Lynch IV",
    "home_pitcher_id": 663738,
    "home_pitcher_stats": {
      "wins": 6,
      "losses": 6,
      "era": "4.13"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-04",
        "opp": "vs MIN",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-08-07",
        "opp": "vs CHC",
        "decision": "L",
        "ip": "1.2",
        "er": 3,
        "h": 3,
        "bb": 2,
        "so": 2,
        "era": 16.2
      },
      {
        "date": "2026-08-12",
        "opp": "@ LAD",
        "decision": "L",
        "ip": "2.1",
        "er": 3,
        "h": 3,
        "bb": 0,
        "so": 3,
        "era": 11.57
      },
      {
        "date": "2026-08-18",
        "opp": "vs OAK",
        "decision": null,
        "ip": "4.0",
        "er": 1,
        "h": 2,
        "bb": 2,
        "so": 1,
        "era": 2.25
      },
      {
        "date": "2026-08-23",
        "opp": "vs DET",
        "decision": null,
        "ip": "4.1",
        "er": 2,
        "h": 4,
        "bb": 1,
        "so": 2,
        "era": 4.15
      },
      {
        "date": "2026-08-29",
        "opp": "@ CLE",
        "decision": null,
        "ip": "3.1",
        "er": 2,
        "h": 3,
        "bb": 2,
        "so": 3,
        "era": 5.4
      },
      {
        "date": "2026-09-04",
        "opp": "vs TOR",
        "decision": "L",
        "ip": "5.0",
        "er": 5,
        "h": 7,
        "bb": 1,
        "so": 0,
        "era": 9.0
      },
      {
        "date": "2026-09-09",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 4,
        "bb": 2,
        "so": 3,
        "era": 3.6
      },
      {
        "date": "2026-09-16",
        "opp": "@ HOU",
        "decision": "W",
        "ip": "5.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs CWS",
        "decision": "L",
        "ip": "4.0",
        "er": 9,
        "h": 7,
        "bb": 3,
        "so": 2,
        "era": 20.25
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 52.0,
      "home": 48.0
    },
    "expected_score": {
      "away": 2.8,
      "home": 2.2
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "L",
      "home_handedness": "L",
      "bullpen_game": true,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.33,
        "bp": 0.32,
        "bat": 0.28,
        "sit": 0.15
      },
      "away": {
        "sp_score": 30.0,
        "sp_detail": {
          "era": 5.11,
          "whip": 1.4,
          "k9": 6.9,
          "avg_ip": 4.9,
          "qs_rate": 30.0,
          "last3_era": 7.29,
          "last_start_era": 9.0,
          "last2_eras": [
            0.0,
            9.0
          ],
          "recent_avg_era": 9.0,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 31,
          "fb_velo": 95.9,
          "pitch_arsenal": [
            "SW",
            "FB",
            "SI",
            "CT",
            "CH",
            "SL"
          ],
          "secondary_pitches": [
            "SW",
            "CH",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 23.4,
              "velo": 82.7
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 21.8,
              "velo": 95.9
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 17.9,
              "velo": 95.3
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 16.6,
              "velo": 91.7
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 14.0,
              "velo": 86.1
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 6.3,
              "velo": 86.3
            }
          ]
        },
        "bp_score": 65.4,
        "bp_detail": {
          "bullpen_era": 3.03,
          "recent_era": 4.15,
          "recent_appearances": 11,
          "recent_ip": 13.0,
          "team_era": 3.97,
          "bp_ip": 232.0,
          "bp_count": 4,
          "closer_era": 2.3,
          "closer_name": "Grant Taylor"
        },
        "bat_score": 36.7,
        "bat_detail": {
          "recent_avg": 0.22,
          "runs_per_g": 5.3,
          "hr_per_g": 1.0,
          "bb_per_g": 3.0,
          "season_ops": 0.73,
          "season_slg": null,
          "season_avg": 0.22,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Miguel Vargas(0.965)",
            "Tommy Pham(0.641)",
            "Randal Grichuk(0.970)",
            "Munetaka Murakami(0.789)",
            "Chase Meidroth(0.818)",
            "Kyle Teel(0.518)",
            "Brenton Doyle(0.755)",
            "Colson Montgomery(0.559)",
            "Braden Montgomery(0.553)"
          ],
          "lineup_players": [
            {
              "id": 678246,
              "name": "Miguel Vargas",
              "ops": 0.965,
              "avg": 0.312
            },
            {
              "id": 502054,
              "name": "Tommy Pham",
              "ops": 0.641,
              "avg": 0.188
            },
            {
              "id": 545341,
              "name": "Randal Grichuk",
              "ops": 0.97,
              "avg": 0.353
            },
            {
              "id": 808959,
              "name": "Munetaka Murakami",
              "ops": 0.789,
              "avg": 0.143
            },
            {
              "id": 805367,
              "name": "Chase Meidroth",
              "ops": 0.818,
              "avg": 0.297
            },
            {
              "id": 691019,
              "name": "Kyle Teel",
              "ops": 0.518,
              "avg": 0.071
            },
            {
              "id": 686668,
              "name": "Brenton Doyle",
              "ops": 0.755,
              "avg": 0.345
            },
            {
              "id": 695657,
              "name": "Colson Montgomery",
              "ops": 0.559,
              "avg": 0.115
            },
            {
              "id": 695731,
              "name": "Braden Montgomery",
              "ops": 0.553,
              "avg": 0.16
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 6.6,
          "last5_avg": 0.248,
          "home_split": {
            "recent_avg": 0.207,
            "runs_per_g": 4.5,
            "hr_per_g": 1.3,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.223,
            "runs_per_g": 4.2,
            "hr_per_g": 0.9,
            "n_games": 10
          }
        },
        "sit_score": 59.2,
        "total": 43.7
      },
      "home": {
        "sp_score": 32.0,
        "sp_detail": {
          "era": 6.81,
          "whip": 1.4,
          "k9": 5.0,
          "avg_ip": 3.6,
          "qs_rate": 0.0,
          "last3_era": 7.25,
          "last_start_era": 20.25,
          "last2_eras": [
            0.0,
            20.25
          ],
          "recent_avg_era": 20.25,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 29,
          "fb_velo": 94.5,
          "pitch_arsenal": [
            "SI",
            "CH",
            "SL",
            "FB",
            "KC"
          ],
          "secondary_pitches": [
            "CH",
            "SL",
            "KC"
          ],
          "pitches_detail": [
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 31.2,
              "velo": 94.0
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 24.8,
              "velo": 85.5
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 19.2,
              "velo": 87.3
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 15.4,
              "velo": 94.5
            },
            {
              "code": "KC",
              "abbr": "KC",
              "pct": 9.4,
              "velo": 81.8
            }
          ]
        },
        "bp_score": 25.0,
        "bp_detail": {
          "bullpen_era": 4.92,
          "recent_era": 7.78,
          "recent_appearances": 21,
          "recent_ip": 19.7,
          "team_era": 4.24,
          "bp_ip": 272.3,
          "bp_count": 9,
          "closer_era": 5.96,
          "closer_name": "Lucas Erceg"
        },
        "bat_score": 30.7,
        "bat_detail": {
          "recent_avg": 0.254,
          "runs_per_g": 3.5,
          "hr_per_g": 1.0,
          "bb_per_g": 3.0,
          "season_ops": 0.708,
          "season_slg": null,
          "season_avg": 0.254,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Carter Jensen(0.850)",
            "Bobby Witt(0.768)",
            "Jac Caglianone(0.793)",
            "Maikel Garcia(0.659)",
            "Vinnie Pasquantino(0.966)",
            "Salvador Perez(0.515)",
            "Michael Massey(0.671)",
            "Nick Loftin(0.748)",
            "Kyle Isbel(0.404)"
          ],
          "lineup_players": [
            {
              "id": 695600,
              "name": "Carter Jensen",
              "ops": 0.85,
              "avg": 0.306
            },
            {
              "id": 677951,
              "name": "Bobby Witt",
              "ops": 0.768,
              "avg": 0.314
            },
            {
              "id": 695506,
              "name": "Jac Caglianone",
              "ops": 0.793,
              "avg": 0.318
            },
            {
              "id": 672580,
              "name": "Maikel Garcia",
              "ops": 0.659,
              "avg": 0.212
            },
            {
              "id": 686469,
              "name": "Vinnie Pasquantino",
              "ops": 0.966,
              "avg": 0.355
            },
            {
              "id": 521692,
              "name": "Salvador Perez",
              "ops": 0.515,
              "avg": 0.179
            },
            {
              "id": 686681,
              "name": "Michael Massey",
              "ops": 0.671,
              "avg": 0.2
            },
            {
              "id": 679845,
              "name": "Nick Loftin",
              "ops": 0.748,
              "avg": 0.29
            },
            {
              "id": 664728,
              "name": "Kyle Isbel",
              "ops": 0.404,
              "avg": 0.115
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 4.8,
          "last5_avg": 0.308,
          "home_split": {
            "recent_avg": 0.25,
            "runs_per_g": 4.7,
            "hr_per_g": 1.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.216,
            "runs_per_g": 2.8,
            "hr_per_g": 0.8,
            "n_games": 10
          }
        },
        "sit_score": 36.5,
        "total": 30.2
      }
    },
    "scores": {
      "away_offense": 36.7,
      "away_defense": 47.7,
      "home_offense": 30.7,
      "home_defense": 27.4
    },
    "actual_score": {
      "away": 14,
      "home": 11
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Chicago White Sox",
    "model_winner": "Chicago White Sox",
    "model_correct": true,
    "notes": "Chicago White Sox IL: Brooks Baldwin, Joey Bart, Jordan Leasure, Ky Bush, Mike Vasil 외 다수 / Kansas City Royals IL: Alec Marsh, Beck Way, Carlos Estévez, Cole Ragans, Connor Seabold 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "SP 점수 차이 2.0pt (≤5.0pt) / 최고 확률 52.0% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "4:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824624,
    "away": "Miami Marlins",
    "home": "Chicago Cubs",
    "away_standing": {
      "div_rank": 3,
      "div_name": "NL East",
      "wins": 77,
      "losses": 80,
      "games_back": "15.0"
    },
    "home_standing": {
      "div_rank": 2,
      "div_name": "NL Central",
      "wins": 87,
      "losses": 70,
      "games_back": "11.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "W",
          "score": "4-2",
          "home": false
        },
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "4-3",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-8",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "6-7",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "3-7",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -3
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "L",
          "score": "3-6",
          "home": true
        },
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "8-4",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "4-6",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "5-2",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "9-1",
          "home": false
        }
      ],
      "wins": 3,
      "losses": 2,
      "streak": 2
    },
    "away_pitcher": "Janson Junk",
    "away_pitcher_id": 676083,
    "away_pitcher_stats": {
      "wins": 6,
      "losses": 9,
      "era": "4.31"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-06",
        "opp": "@ ATL",
        "decision": "L",
        "ip": "3.0",
        "er": 2,
        "h": 5,
        "bb": 1,
        "so": 1,
        "era": 6.0
      },
      {
        "date": "2026-08-12",
        "opp": "vs PIT",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 2,
        "era": 1.5
      },
      {
        "date": "2026-08-17",
        "opp": "@ PHI",
        "decision": "L",
        "ip": "5.0",
        "er": 2,
        "h": 8,
        "bb": 2,
        "so": 2,
        "era": 3.6
      },
      {
        "date": "2026-08-23",
        "opp": "vs WSH",
        "decision": null,
        "ip": "4.2",
        "er": 1,
        "h": 2,
        "bb": 0,
        "so": 2,
        "era": 1.93
      },
      {
        "date": "2026-08-26",
        "opp": "vs BOS",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-30",
        "opp": "@ WSH",
        "decision": null,
        "ip": "4.2",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 2,
        "era": 1.93
      },
      {
        "date": "2026-09-04",
        "opp": "vs CHC",
        "decision": "L",
        "ip": "5.0",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 1.8
      },
      {
        "date": "2026-09-09",
        "opp": "vs NYM",
        "decision": null,
        "ip": "3.0",
        "er": 8,
        "h": 8,
        "bb": 1,
        "so": 0,
        "era": 24.0
      },
      {
        "date": "2026-09-15",
        "opp": "@ OAK",
        "decision": null,
        "ip": "3.0",
        "er": 1,
        "h": 5,
        "bb": 1,
        "so": 4,
        "era": 3.0
      },
      {
        "date": "2026-09-22",
        "opp": "@ CHC",
        "decision": null,
        "ip": "5.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 6,
        "era": 0.0
      }
    ],
    "home_pitcher": "Shota Imanaga",
    "home_pitcher_id": 684007,
    "home_pitcher_stats": {
      "wins": 11,
      "losses": 10,
      "era": "3.74"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-07-31",
        "opp": "vs NYY",
        "decision": "L",
        "ip": "6.2",
        "er": 2,
        "h": 4,
        "bb": 0,
        "so": 6,
        "era": 2.7
      },
      {
        "date": "2026-08-05",
        "opp": "vs LAD",
        "decision": "W",
        "ip": "5.0",
        "er": 1,
        "h": 8,
        "bb": 0,
        "so": 6,
        "era": 1.8
      },
      {
        "date": "2026-08-11",
        "opp": "@ WSH",
        "decision": null,
        "ip": "4.2",
        "er": 4,
        "h": 6,
        "bb": 0,
        "so": 5,
        "era": 7.71
      },
      {
        "date": "2026-08-17",
        "opp": "vs CWS",
        "decision": null,
        "ip": "6.0",
        "er": 3,
        "h": 2,
        "bb": 3,
        "so": 10,
        "era": 4.5
      },
      {
        "date": "2026-08-23",
        "opp": "@ SEA",
        "decision": null,
        "ip": "2.0",
        "er": 1,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 4.5
      },
      {
        "date": "2026-08-30",
        "opp": "vs CIN",
        "decision": "L",
        "ip": "5.0",
        "er": 6,
        "h": 8,
        "bb": 1,
        "so": 4,
        "era": 10.8
      },
      {
        "date": "2026-09-04",
        "opp": "@ MIA",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 0,
        "so": 4,
        "era": 1.5
      },
      {
        "date": "2026-09-11",
        "opp": "vs PIT",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 3.0
      },
      {
        "date": "2026-09-16",
        "opp": "vs ATL",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 0,
        "so": 8,
        "era": 1.5
      },
      {
        "date": "2026-09-22",
        "opp": "vs MIA",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 4,
        "bb": 0,
        "so": 7,
        "era": 1.8
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 38.6,
      "home": 61.4
    },
    "expected_score": {
      "away": 1.7,
      "home": 3.0
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "L",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 32.0,
        "sp_detail": {
          "era": 3.79,
          "whip": 1.26,
          "k9": 5.1,
          "avg_ip": 4.0,
          "qs_rate": 10.0,
          "last3_era": 4.79,
          "last_start_era": 0.0,
          "last2_eras": [
            3.0,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 30,
          "fb_velo": 94.1,
          "pitch_arsenal": [
            "FB",
            "SL",
            "CH",
            "SW",
            "CB",
            "SI"
          ],
          "secondary_pitches": [
            "SL",
            "CH",
            "SW",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 34.6,
              "velo": 94.1
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 19.5,
              "velo": 86.3
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 17.8,
              "velo": 87.3
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 14.4,
              "velo": 80.1
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 9.3,
              "velo": 80.7
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 4.0,
              "velo": 93.6
            }
          ]
        },
        "bp_score": 37.6,
        "bp_detail": {
          "bullpen_era": 4.25,
          "recent_era": 6.1,
          "recent_appearances": 19,
          "recent_ip": 20.7,
          "team_era": 4.07,
          "bp_ip": 351.7,
          "bp_count": 9,
          "closer_era": 3.39,
          "closer_name": "Calvin Faucher"
        },
        "bat_score": 27.0,
        "bat_detail": {
          "recent_avg": 0.214,
          "runs_per_g": 4.4,
          "hr_per_g": 1.1,
          "bb_per_g": 3.0,
          "season_ops": 0.66,
          "season_slg": null,
          "season_avg": 0.214,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Heriberto Hernández(0.690)",
            "Otto Lopez(0.736)",
            "Kyle Stowers(0.682)",
            "Javier Sanoja(0.868)",
            "Agustín Ramírez(0.832)",
            "Esteury Ruiz(0.500)",
            "Griffin Conine(0.758)",
            "Deyvison De Los Santos(0.376)",
            "Jared Serna(0.499)"
          ],
          "lineup_players": [
            {
              "id": 681715,
              "name": "Heriberto Hernández",
              "ops": 0.69,
              "avg": 0.275
            },
            {
              "id": 672640,
              "name": "Otto Lopez",
              "ops": 0.736,
              "avg": 0.243
            },
            {
              "id": 669065,
              "name": "Kyle Stowers",
              "ops": 0.682,
              "avg": 0.179
            },
            {
              "id": 691594,
              "name": "Javier Sanoja",
              "ops": 0.868,
              "avg": 0.3
            },
            {
              "id": 682663,
              "name": "Agustín Ramírez",
              "ops": 0.832,
              "avg": 0.321
            },
            {
              "id": 665923,
              "name": "Esteury Ruiz",
              "ops": 0.5,
              "avg": 0.103
            },
            {
              "id": 665052,
              "name": "Griffin Conine",
              "ops": 0.758,
              "avg": 0.243
            },
            {
              "id": 691277,
              "name": "Deyvison De Los Santos",
              "ops": 0.376,
              "avg": 0.111
            },
            {
              "id": 691858,
              "name": "Jared Serna",
              "ops": 0.499,
              "avg": 0.154
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 3.8,
          "last5_avg": 0.233,
          "home_split": {
            "recent_avg": 0.268,
            "runs_per_g": 5.5,
            "hr_per_g": 0.9,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.243,
            "runs_per_g": 4.6,
            "hr_per_g": 1.4,
            "n_games": 10
          }
        },
        "sit_score": 51.9,
        "total": 37.4
      },
      "home": {
        "sp_score": 61.4,
        "sp_detail": {
          "era": 3.78,
          "whip": 1.09,
          "k9": 10.0,
          "avg_ip": 5.2,
          "qs_rate": 50.0,
          "last3_era": 3.54,
          "last_start_era": 1.8,
          "last2_eras": [
            1.5,
            1.8
          ],
          "recent_avg_era": 1.8,
          "recent_bad_start": false,
          "trend": "stable",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 33,
          "fb_velo": 91.6,
          "pitch_arsenal": [
            "FB",
            "FS",
            "SW",
            "SI"
          ],
          "secondary_pitches": [
            "FS",
            "SW"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 40.2,
              "velo": 91.6
            },
            {
              "code": "FS",
              "abbr": "FS",
              "pct": 34.5,
              "velo": 82.8
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 14.5,
              "velo": 81.0
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 5.9,
              "velo": 90.2
            }
          ]
        },
        "bp_score": 57.0,
        "bp_detail": {
          "bullpen_era": 3.76,
          "recent_era": 3.18,
          "recent_appearances": 11,
          "recent_ip": 11.3,
          "team_era": 4.18,
          "bp_ip": 272.7,
          "bp_count": 5,
          "closer_era": 3.48,
          "closer_name": "Jacob Webb"
        },
        "bat_score": 38.6,
        "bat_detail": {
          "recent_avg": 0.258,
          "runs_per_g": 4.5,
          "hr_per_g": 1.3,
          "bb_per_g": 3.0,
          "season_ops": 0.767,
          "season_slg": null,
          "season_avg": 0.258,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Pete Crow-Armstrong(0.988)",
            "Seiya Suzuki(0.752)",
            "Michael Busch(0.839)",
            "Michael Conforto(0.768)",
            "Ian Happ(0.809)",
            "Nico Hoerner(0.578)",
            "Pedro Ramírez(0.694)",
            "Carson Kelly(0.712)",
            "Dansby Swanson(0.767)"
          ],
          "lineup_players": [
            {
              "id": 691718,
              "name": "Pete Crow-Armstrong",
              "ops": 0.988,
              "avg": 0.286
            },
            {
              "id": 673548,
              "name": "Seiya Suzuki",
              "ops": 0.752,
              "avg": 0.25
            },
            {
              "id": 683737,
              "name": "Michael Busch",
              "ops": 0.839,
              "avg": 0.276
            },
            {
              "id": 624424,
              "name": "Michael Conforto",
              "ops": 0.768,
              "avg": 0.258
            },
            {
              "id": 664023,
              "name": "Ian Happ",
              "ops": 0.809,
              "avg": 0.259
            },
            {
              "id": 663538,
              "name": "Nico Hoerner",
              "ops": 0.578,
              "avg": 0.206
            },
            {
              "id": 699393,
              "name": "Pedro Ramírez",
              "ops": 0.694,
              "avg": 0.243
            },
            {
              "id": 608348,
              "name": "Carson Kelly",
              "ops": 0.712,
              "avg": 0.31
            },
            {
              "id": 621020,
              "name": "Dansby Swanson",
              "ops": 0.767,
              "avg": 0.233
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 4.0,
          "last5_avg": 0.247,
          "home_split": {
            "recent_avg": 0.236,
            "runs_per_g": 4.6,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.236,
            "runs_per_g": 4.5,
            "hr_per_g": 1.1,
            "n_games": 10
          }
        },
        "sit_score": 53.8,
        "total": 57.2
      }
    },
    "scores": {
      "away_offense": 27.0,
      "away_defense": 34.8,
      "home_offense": 38.6,
      "home_defense": 59.2
    },
    "actual_score": {
      "away": 8,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Miami Marlins",
    "model_winner": "Chicago Cubs",
    "model_correct": false,
    "notes": "Miami Marlins IL: Adam Mazur, Andrew Nardi, Anthony Bender, Max Meyer, Owen Caissie 외 다수 / Chicago Cubs IL: Ben Brown, Cade Horton, Gavin Hollowell, Hunter Harvey, Justin Steele 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "5:05 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 822840,
    "away": "New York Mets",
    "home": "Texas Rangers",
    "away_standing": {
      "div_rank": 5,
      "div_name": "NL East",
      "wins": 72,
      "losses": 85,
      "games_back": "20.0"
    },
    "home_standing": {
      "div_rank": 2,
      "div_name": "AL West",
      "wins": 78,
      "losses": 79,
      "games_back": "-"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "1-7",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "0-3",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "6-3",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "10-3",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "2-7",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -1
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "7-3",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "3-4",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "7-1",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "6-2",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "2-7",
          "home": true
        }
      ],
      "wins": 3,
      "losses": 2,
      "streak": -1
    },
    "away_pitcher": "Sean Manaea",
    "away_pitcher_id": 640455,
    "away_pitcher_stats": {
      "wins": 6,
      "losses": 7,
      "era": "4.67"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-29",
        "opp": "vs ATL",
        "decision": null,
        "ip": "6.0",
        "er": 2,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 3.0
      },
      {
        "date": "2026-08-04",
        "opp": "@ CLE",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 7,
        "bb": 5,
        "so": 7,
        "era": 3.0
      },
      {
        "date": "2026-08-09",
        "opp": "@ PIT",
        "decision": "W",
        "ip": "7.0",
        "er": 1,
        "h": 3,
        "bb": 0,
        "so": 11,
        "era": 1.29
      },
      {
        "date": "2026-08-15",
        "opp": "vs WSH",
        "decision": null,
        "ip": "6.1",
        "er": 4,
        "h": 6,
        "bb": 2,
        "so": 7,
        "era": 5.68
      },
      {
        "date": "2026-08-21",
        "opp": "@ CWS",
        "decision": null,
        "ip": "6.0",
        "er": 4,
        "h": 5,
        "bb": 2,
        "so": 4,
        "era": 6.0
      },
      {
        "date": "2026-08-27",
        "opp": "vs MIL",
        "decision": "L",
        "ip": "5.2",
        "er": 6,
        "h": 8,
        "bb": 1,
        "so": 3,
        "era": 9.53
      },
      {
        "date": "2026-09-01",
        "opp": "@ TB",
        "decision": "L",
        "ip": "5.2",
        "er": 5,
        "h": 11,
        "bb": 1,
        "so": 2,
        "era": 7.94
      },
      {
        "date": "2026-09-08",
        "opp": "@ MIA",
        "decision": "W",
        "ip": "5.0",
        "er": 4,
        "h": 5,
        "bb": 0,
        "so": 4,
        "era": 7.2
      },
      {
        "date": "2026-09-15",
        "opp": "vs BAL",
        "decision": null,
        "ip": "5.1",
        "er": 2,
        "h": 4,
        "bb": 0,
        "so": 6,
        "era": 3.38
      },
      {
        "date": "2026-09-22",
        "opp": "@ TEX",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 3,
        "bb": 2,
        "so": 9,
        "era": 3.0
      }
    ],
    "home_pitcher": "MacKenzie Gore",
    "home_pitcher_id": 669022,
    "home_pitcher_stats": {
      "wins": 8,
      "losses": 11,
      "era": "4.59"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-04",
        "opp": "vs SF",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 3,
        "bb": 3,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-08-10",
        "opp": "@ LAA",
        "decision": null,
        "ip": "5.1",
        "er": 1,
        "h": 4,
        "bb": 2,
        "so": 9,
        "era": 1.69
      },
      {
        "date": "2026-08-15",
        "opp": "@ OAK",
        "decision": "W",
        "ip": "5.2",
        "er": 3,
        "h": 8,
        "bb": 2,
        "so": 5,
        "era": 4.76
      },
      {
        "date": "2026-08-21",
        "opp": "vs LAA",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-08-26",
        "opp": "@ CWS",
        "decision": "L",
        "ip": "3.2",
        "er": 4,
        "h": 8,
        "bb": 4,
        "so": 4,
        "era": 9.82
      },
      {
        "date": "2026-09-01",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "5.1",
        "er": 1,
        "h": 5,
        "bb": 4,
        "so": 4,
        "era": 1.69
      },
      {
        "date": "2026-09-06",
        "opp": "vs TB",
        "decision": null,
        "ip": "3.0",
        "er": 5,
        "h": 8,
        "bb": 3,
        "so": 1,
        "era": 15.0
      },
      {
        "date": "2026-09-11",
        "opp": "@ OAK",
        "decision": "L",
        "ip": "4.0",
        "er": 4,
        "h": 5,
        "bb": 3,
        "so": 2,
        "era": 9.0
      },
      {
        "date": "2026-09-16",
        "opp": "vs BOS",
        "decision": null,
        "ip": "4.0",
        "er": 3,
        "h": 2,
        "bb": 3,
        "so": 0,
        "era": 6.75
      },
      {
        "date": "2026-09-22",
        "opp": "vs NYM",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 2,
        "era": 1.8
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 48.6,
      "home": 51.4
    },
    "expected_score": {
      "away": 2.4,
      "home": 2.1
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "L",
      "home_handedness": "L",
      "bullpen_game": true,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.3,
        "bat": 0.25,
        "sit": 0.15
      },
      "away": {
        "sp_score": 38.1,
        "sp_detail": {
          "era": 4.88,
          "whip": 1.22,
          "k9": 8.7,
          "avg_ip": 5.9,
          "qs_rate": 40.0,
          "last3_era": 6.18,
          "last_start_era": 3.0,
          "last2_eras": [
            3.38,
            3.0
          ],
          "recent_avg_era": 3.0,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 34,
          "fb_velo": 91.3,
          "pitch_arsenal": [
            "FB",
            "SW",
            "SI",
            "CT",
            "CH"
          ],
          "secondary_pitches": [
            "SW",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 38.7,
              "velo": 91.3
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 30.7,
              "velo": 75.8
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 18.7,
              "velo": 90.4
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 7.6,
              "velo": 84.6
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 4.3,
              "velo": 84.1
            }
          ]
        },
        "bp_score": 49.6,
        "bp_detail": {
          "bullpen_era": 4.34,
          "recent_era": 2.33,
          "recent_appearances": 17,
          "recent_ip": 27.0,
          "team_era": 4.28,
          "bp_ip": 305.0,
          "bp_count": 10,
          "closer_era": 4.66,
          "closer_name": "Devin Williams"
        },
        "bat_score": 35.7,
        "bat_detail": {
          "recent_avg": 0.258,
          "runs_per_g": 4.3,
          "hr_per_g": 1.2,
          "bb_per_g": 3.0,
          "season_ops": 0.725,
          "season_slg": null,
          "season_avg": 0.258,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Francisco Lindor(0.704)",
            "Juan Soto(0.865)",
            "Bo Bichette(0.630)",
            "Mark Vientos(0.742)",
            "Carson Benge(0.880)",
            "Marcus Semien(0.573)",
            "Francisco Alvarez(0.698)",
            "A.J. Ewing(0.718)",
            "Luis Torrens(0.718)"
          ],
          "lineup_players": [
            {
              "id": 596019,
              "name": "Francisco Lindor",
              "ops": 0.704,
              "avg": 0.19
            },
            {
              "id": 665742,
              "name": "Juan Soto",
              "ops": 0.865,
              "avg": 0.286
            },
            {
              "id": 666182,
              "name": "Bo Bichette",
              "ops": 0.63,
              "avg": 0.227
            },
            {
              "id": 668901,
              "name": "Mark Vientos",
              "ops": 0.742,
              "avg": 0.273
            },
            {
              "id": 701807,
              "name": "Carson Benge",
              "ops": 0.88,
              "avg": 0.341
            },
            {
              "id": 543760,
              "name": "Marcus Semien",
              "ops": 0.573,
              "avg": 0.205
            },
            {
              "id": 682626,
              "name": "Francisco Alvarez",
              "ops": 0.698,
              "avg": 0.256
            },
            {
              "id": 805999,
              "name": "A.J. Ewing",
              "ops": 0.718,
              "avg": 0.286
            },
            {
              "id": 620443,
              "name": "Luis Torrens",
              "ops": 0.718,
              "avg": 0.259
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 4.8,
          "last5_avg": 0.279,
          "home_split": {
            "recent_avg": 0.248,
            "runs_per_g": 4.4,
            "hr_per_g": 1.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.275,
            "runs_per_g": 6.8,
            "hr_per_g": 1.5,
            "n_games": 10
          }
        },
        "sit_score": 47.4,
        "total": 43.2
      },
      "home": {
        "sp_score": 29.9,
        "sp_detail": {
          "era": 4.12,
          "whip": 1.56,
          "k9": 7.1,
          "avg_ip": 4.8,
          "qs_rate": 20.0,
          "last3_era": 5.91,
          "last_start_era": 1.8,
          "last2_eras": [
            6.75,
            1.8
          ],
          "recent_avg_era": 1.8,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 27,
          "fb_velo": 95.1,
          "pitch_arsenal": [
            "FB",
            "CB",
            "CH",
            "SL",
            "CT",
            "SI"
          ],
          "secondary_pitches": [
            "CB",
            "CH",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 47.4,
              "velo": 95.1
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 18.1,
              "velo": 81.5
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 13.0,
              "velo": 86.2
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 9.9,
              "velo": 85.7
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 6.3,
              "velo": 89.9
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 5.3,
              "velo": 95.1
            }
          ]
        },
        "bp_score": 70.6,
        "bp_detail": {
          "bullpen_era": 3.31,
          "recent_era": 2.7,
          "recent_appearances": 15,
          "recent_ip": 20.0,
          "team_era": 3.95,
          "bp_ip": 323.3,
          "bp_count": 8,
          "closer_era": 2.08,
          "closer_name": "Jacob Latz"
        },
        "bat_score": 28.0,
        "bat_detail": {
          "recent_avg": 0.241,
          "runs_per_g": 4.6,
          "hr_per_g": 0.5,
          "bb_per_g": 3.0,
          "season_ops": 0.707,
          "season_slg": null,
          "season_avg": 0.241,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Wyatt Langford(0.785)",
            "Corey Seager(0.684)",
            "Josh Jung(0.836)",
            "Jake Burger(0.658)",
            "Brandon Nimmo(0.679)",
            "Ezequiel Duran(0.589)",
            "Justin Foscue(0.720)",
            "Elias Díaz(0.604)",
            "Danny Jansen(0.805)"
          ],
          "lineup_players": [
            {
              "id": 694671,
              "name": "Wyatt Langford",
              "ops": 0.785,
              "avg": 0.226
            },
            {
              "id": 608369,
              "name": "Corey Seager",
              "ops": 0.684,
              "avg": 0.238
            },
            {
              "id": 673962,
              "name": "Josh Jung",
              "ops": 0.836,
              "avg": 0.308
            },
            {
              "id": 669394,
              "name": "Jake Burger",
              "ops": 0.658,
              "avg": 0.147
            },
            {
              "id": 607043,
              "name": "Brandon Nimmo",
              "ops": 0.679,
              "avg": 0.237
            },
            {
              "id": 677649,
              "name": "Ezequiel Duran",
              "ops": 0.589,
              "avg": 0.167
            },
            {
              "id": 679822,
              "name": "Justin Foscue",
              "ops": 0.72,
              "avg": 0.25
            },
            {
              "id": 553869,
              "name": "Elias Díaz",
              "ops": 0.604,
              "avg": 0.24
            },
            {
              "id": 643376,
              "name": "Danny Jansen",
              "ops": 0.805,
              "avg": 0.357
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 4.2,
          "last5_avg": 0.181,
          "home_split": {
            "recent_avg": 0.226,
            "runs_per_g": 4.9,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.232,
            "runs_per_g": 4.4,
            "hr_per_g": 1.4,
            "n_games": 10
          }
        },
        "sit_score": 54.0,
        "total": 44.5
      }
    },
    "scores": {
      "away_offense": 35.7,
      "away_defense": 43.9,
      "home_offense": 28.0,
      "home_defense": 50.2
    },
    "actual_score": {
      "away": 6,
      "home": 3
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "New York Mets",
    "model_winner": "Texas Rangers",
    "model_correct": false,
    "notes": "New York Mets IL: Christian Scott, Daniel Duarte, Jared Young, Jorge Polanco, Reed Garrett 외 다수 / Texas Rangers IL: Cal Quantrill, Carter Baumler, Cole Winn, Jack Leiter, Jalen Beeks 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 51.4% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "5:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824302,
    "away": "Arizona Diamondbacks",
    "home": "Colorado Rockies",
    "away_standing": {
      "div_rank": 3,
      "div_name": "NL West",
      "wins": 83,
      "losses": 74,
      "games_back": "14.0"
    },
    "home_standing": {
      "div_rank": 5,
      "div_name": "NL West",
      "wins": 57,
      "losses": 100,
      "games_back": "40.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "L",
          "score": "2-4",
          "home": true
        },
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "3-4",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-9",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "5-3",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "8-4",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": 2
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "3-9",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "2-9",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "4-5",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "5-3",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "1-2",
          "home": true
        }
      ],
      "wins": 1,
      "losses": 4,
      "streak": -1
    },
    "away_pitcher": "Michael Soroka",
    "away_pitcher_id": 647336,
    "away_pitcher_stats": {
      "wins": 9,
      "losses": 5,
      "era": "3.31"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-06-02",
        "opp": "vs LAD",
        "decision": "L",
        "ip": "6.0",
        "er": 4,
        "h": 6,
        "bb": 1,
        "so": 6,
        "era": 6.0
      },
      {
        "date": "2026-06-07",
        "opp": "vs WSH",
        "decision": "W",
        "ip": "7.0",
        "er": 1,
        "h": 3,
        "bb": 2,
        "so": 6,
        "era": 1.29
      },
      {
        "date": "2026-06-13",
        "opp": "@ CIN",
        "decision": null,
        "ip": "7.0",
        "er": 1,
        "h": 2,
        "bb": 0,
        "so": 7,
        "era": 1.29
      },
      {
        "date": "2026-06-19",
        "opp": "vs MIN",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 2,
        "bb": 0,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-10",
        "opp": "vs COL",
        "decision": null,
        "ip": "4.1",
        "er": 0,
        "h": 3,
        "bb": 3,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-08-16",
        "opp": "@ ATL",
        "decision": null,
        "ip": "5.2",
        "er": 2,
        "h": 6,
        "bb": 1,
        "so": 5,
        "era": 3.18
      },
      {
        "date": "2026-08-22",
        "opp": "vs CIN",
        "decision": "L",
        "ip": "3.1",
        "er": 6,
        "h": 6,
        "bb": 3,
        "so": 3,
        "era": 16.2
      },
      {
        "date": "2026-09-09",
        "opp": "@ KC",
        "decision": "L",
        "ip": "4.0",
        "er": 3,
        "h": 3,
        "bb": 1,
        "so": 4,
        "era": 6.75
      },
      {
        "date": "2026-09-15",
        "opp": "vs MIA",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 2,
        "bb": 1,
        "so": 6,
        "era": 1.8
      },
      {
        "date": "2026-09-22",
        "opp": "@ COL",
        "decision": "W",
        "ip": "7.0",
        "er": 1,
        "h": 2,
        "bb": 1,
        "so": 9,
        "era": 1.29
      }
    ],
    "home_pitcher": "Kyle Freeland",
    "home_pitcher_id": 607536,
    "home_pitcher_stats": {
      "wins": 4,
      "losses": 11,
      "era": "6.58"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-07-06",
        "opp": "@ LAD",
        "decision": null,
        "ip": "5.0",
        "er": 6,
        "h": 9,
        "bb": 0,
        "so": 2,
        "era": 10.8
      },
      {
        "date": "2026-07-11",
        "opp": "@ SF",
        "decision": "L",
        "ip": "6.0",
        "er": 4,
        "h": 6,
        "bb": 0,
        "so": 9,
        "era": 6.0
      },
      {
        "date": "2026-07-20",
        "opp": "vs WSH",
        "decision": "L",
        "ip": "6.0",
        "er": 4,
        "h": 7,
        "bb": 2,
        "so": 4,
        "era": 6.0
      },
      {
        "date": "2026-07-26",
        "opp": "@ MIL",
        "decision": "L",
        "ip": "5.1",
        "er": 5,
        "h": 6,
        "bb": 1,
        "so": 2,
        "era": 8.44
      },
      {
        "date": "2026-08-02",
        "opp": "vs KC",
        "decision": "W",
        "ip": "9.0",
        "er": 1,
        "h": 5,
        "bb": 2,
        "so": 8,
        "era": 1.0
      },
      {
        "date": "2026-08-08",
        "opp": "@ STL",
        "decision": null,
        "ip": "4.1",
        "er": 1,
        "h": 5,
        "bb": 3,
        "so": 3,
        "era": 2.08
      },
      {
        "date": "2026-08-14",
        "opp": "@ SF",
        "decision": "W",
        "ip": "8.0",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 2,
        "era": 1.12
      },
      {
        "date": "2026-08-19",
        "opp": "vs LAD",
        "decision": null,
        "ip": "3.2",
        "er": 4,
        "h": 6,
        "bb": 1,
        "so": 5,
        "era": 9.82
      },
      {
        "date": "2026-09-15",
        "opp": "vs SD",
        "decision": null,
        "ip": "4.0",
        "er": 3,
        "h": 8,
        "bb": 0,
        "so": 1,
        "era": 6.75
      },
      {
        "date": "2026-09-22",
        "opp": "vs OAK",
        "decision": "L",
        "ip": "3.0",
        "er": 5,
        "h": 9,
        "bb": 1,
        "so": 2,
        "era": 15.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 60.7,
      "home": 39.3
    },
    "expected_score": {
      "away": 3.7,
      "home": 2.5
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "L",
      "bullpen_game": true,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.31,
        "bp": 0.32,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 46.5,
        "sp_detail": {
          "era": 3.4,
          "whip": 0.95,
          "k9": 8.9,
          "avg_ip": 5.0,
          "qs_rate": 30.0,
          "last3_era": 4.68,
          "last_start_era": 1.29,
          "last2_eras": [
            1.8,
            1.29
          ],
          "recent_avg_era": 1.29,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 29,
          "fb_velo": 93.5,
          "pitch_arsenal": [
            "FB",
            "SV",
            "CH",
            "CT",
            "SI"
          ],
          "secondary_pitches": [
            "SV",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 32.4,
              "velo": 93.5
            },
            {
              "code": "SV",
              "abbr": "SV",
              "pct": 32.2,
              "velo": 80.6
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 12.5,
              "velo": 85.9
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 12.0,
              "velo": 89.0
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 10.2,
              "velo": 93.3
            }
          ]
        },
        "bp_score": 73.0,
        "bp_detail": {
          "bullpen_era": 3.04,
          "recent_era": 3.0,
          "recent_appearances": 14,
          "recent_ip": 18.0,
          "team_era": 3.79,
          "bp_ip": 388.0,
          "bp_count": 8,
          "closer_era": 2.15,
          "closer_name": "Juan Morillo"
        },
        "bat_score": 48.6,
        "bat_detail": {
          "recent_avg": 0.286,
          "runs_per_g": 5.2,
          "hr_per_g": 1.5,
          "bb_per_g": 3.0,
          "season_ops": 0.776,
          "season_slg": null,
          "season_avg": 0.286,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Ildemaro Vargas(0.730)",
            "Ketel Marte(0.675)",
            "Gabriel Moreno(0.993)",
            "Geraldo Perdomo(0.816)",
            "Nolan Arenado(1.085)",
            "Jordan Lawlar(0.816)",
            "Corbin Carroll(0.822)",
            "Tim Tawa(0.581)",
            "Jose Fernandez(0.470)"
          ],
          "lineup_players": [
            {
              "id": 545121,
              "name": "Ildemaro Vargas",
              "ops": 0.73,
              "avg": 0.333
            },
            {
              "id": 606466,
              "name": "Ketel Marte",
              "ops": 0.675,
              "avg": 0.227
            },
            {
              "id": 672515,
              "name": "Gabriel Moreno",
              "ops": 0.993,
              "avg": 0.385
            },
            {
              "id": 672695,
              "name": "Geraldo Perdomo",
              "ops": 0.816,
              "avg": 0.378
            },
            {
              "id": 571448,
              "name": "Nolan Arenado",
              "ops": 1.085,
              "avg": 0.432
            },
            {
              "id": 691783,
              "name": "Jordan Lawlar",
              "ops": 0.816,
              "avg": 0.29
            },
            {
              "id": 682998,
              "name": "Corbin Carroll",
              "ops": 0.822,
              "avg": 0.243
            },
            {
              "id": 694374,
              "name": "Tim Tawa",
              "ops": 0.581,
              "avg": 0.133
            },
            {
              "id": 699912,
              "name": "Jose Fernandez",
              "ops": 0.47,
              "avg": 0.15
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 5.0,
          "last5_avg": 0.282,
          "home_split": {
            "recent_avg": 0.255,
            "runs_per_g": 4.6,
            "hr_per_g": 1.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.254,
            "runs_per_g": 4.6,
            "hr_per_g": 1.0,
            "n_games": 10
          }
        },
        "sit_score": 55.4,
        "total": 60.7
      },
      "home": {
        "sp_score": 25.0,
        "sp_detail": {
          "era": 5.63,
          "whip": 1.4,
          "k9": 6.3,
          "avg_ip": 5.4,
          "qs_rate": 20.0,
          "last3_era": 5.48,
          "last_start_era": 15.0,
          "last2_eras": [
            6.75,
            15.0
          ],
          "recent_avg_era": 15.0,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 33,
          "fb_velo": 91.3,
          "pitch_arsenal": [
            "FB",
            "KC",
            "CT",
            "CH",
            "SW",
            "SI"
          ],
          "secondary_pitches": [
            "KC",
            "CH",
            "SW"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 23.5,
              "velo": 91.3
            },
            {
              "code": "KC",
              "abbr": "KC",
              "pct": 21.9,
              "velo": 82.4
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 21.4,
              "velo": 87.6
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 13.5,
              "velo": 86.2
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 10.0,
              "velo": 82.7
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 9.7,
              "velo": 91.1
            }
          ]
        },
        "bp_score": 35.2,
        "bp_detail": {
          "bullpen_era": 5.06,
          "recent_era": 3.0,
          "recent_appearances": 18,
          "recent_ip": 27.0,
          "team_era": 5.5,
          "bp_ip": 375.0,
          "bp_count": 8,
          "closer_era": 5.12,
          "closer_name": "Jordan Romano"
        },
        "bat_score": 38.1,
        "bat_detail": {
          "recent_avg": 0.242,
          "runs_per_g": 4.1,
          "hr_per_g": 0.9,
          "bb_per_g": 3.0,
          "season_ops": 0.733,
          "season_slg": null,
          "season_avg": 0.242,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Jake McCarthy(0.808)",
            "Connor Norby(0.783)",
            "TJ Rumfield(0.771)",
            "Hunter Goodman(0.771)",
            "Cole Carrigg(0.827)",
            "Kyle Karros(0.715)",
            "Mickey Moniak(0.703)",
            "Troy Johnston(0.681)",
            "Zac Veen(0.539)"
          ],
          "lineup_players": [
            {
              "id": 664983,
              "name": "Jake McCarthy",
              "ops": 0.808,
              "avg": 0.275
            },
            {
              "id": 681393,
              "name": "Connor Norby",
              "ops": 0.783,
              "avg": 0.342
            },
            {
              "id": 681198,
              "name": "TJ Rumfield",
              "ops": 0.771,
              "avg": 0.308
            },
            {
              "id": 696100,
              "name": "Hunter Goodman",
              "ops": 0.771,
              "avg": 0.175
            },
            {
              "id": 694249,
              "name": "Cole Carrigg",
              "ops": 0.827,
              "avg": 0.31
            },
            {
              "id": 691720,
              "name": "Kyle Karros",
              "ops": 0.715,
              "avg": 0.205
            },
            {
              "id": 666160,
              "name": "Mickey Moniak",
              "ops": 0.703,
              "avg": 0.176
            },
            {
              "id": 687859,
              "name": "Troy Johnston",
              "ops": 0.681,
              "avg": 0.235
            },
            {
              "id": 691728,
              "name": "Zac Veen",
              "ops": 0.539,
              "avg": 0.15
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.8,
          "last5_avg": 0.243,
          "home_split": {
            "recent_avg": 0.276,
            "runs_per_g": 5.1,
            "hr_per_g": 0.8,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.227,
            "runs_per_g": 2.5,
            "hr_per_g": 0.8,
            "n_games": 10
          }
        },
        "sit_score": 39.9,
        "total": 33.0
      }
    },
    "scores": {
      "away_offense": 48.6,
      "away_defense": 59.8,
      "home_offense": 38.1,
      "home_defense": 31.6
    },
    "actual_score": {
      "away": 7,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Arizona Diamondbacks",
    "model_winner": "Arizona Diamondbacks",
    "model_correct": true,
    "notes": "Arizona Diamondbacks IL: A.J. Puk, Andrew Saalfrank, Cristian Mena, Ryan Thompson, Tyler Locklear / Colorado Rockies IL: Chase Dollander, Kris Bryant, McCade Brown, Pierson Ohl, Welinton Herrera 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "6:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824953,
    "away": "Los Angeles Angels",
    "home": "Athletics",
    "away_standing": {
      "div_rank": 5,
      "div_name": "AL West",
      "wins": 60,
      "losses": 97,
      "games_back": "18.0"
    },
    "home_standing": {
      "div_rank": 4,
      "div_name": "AL West",
      "wins": 62,
      "losses": 95,
      "games_back": "16.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "2-7",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "5-4",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "0-3",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "6-5",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "0-8",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -1
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "3-4",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "1-10",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "3-5",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "6-12",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "0-1",
          "home": false
        }
      ],
      "wins": 0,
      "losses": 5,
      "streak": -5
    },
    "away_pitcher": "Yusei Kikuchi",
    "away_pitcher_id": 579328,
    "away_pitcher_stats": {
      "wins": 1,
      "losses": 6,
      "era": "4.88"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-04-13",
        "opp": "@ NYY",
        "decision": null,
        "ip": "3.1",
        "er": 4,
        "h": 4,
        "bb": 4,
        "so": 3,
        "era": 10.8
      },
      {
        "date": "2026-04-18",
        "opp": "vs SD",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 1,
        "so": 8,
        "era": 0.0
      },
      {
        "date": "2026-04-24",
        "opp": "@ KC",
        "decision": "L",
        "ip": "5.0",
        "er": 5,
        "h": 5,
        "bb": 2,
        "so": 5,
        "era": 9.0
      },
      {
        "date": "2026-04-29",
        "opp": "@ CWS",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-08-23",
        "opp": "@ TEX",
        "decision": "L",
        "ip": "5.0",
        "er": 5,
        "h": 7,
        "bb": 1,
        "so": 4,
        "era": 9.0
      },
      {
        "date": "2026-08-30",
        "opp": "vs PHI",
        "decision": "L",
        "ip": "7.0",
        "er": 2,
        "h": 6,
        "bb": 1,
        "so": 6,
        "era": 2.57
      },
      {
        "date": "2026-09-05",
        "opp": "@ PIT",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 5,
        "bb": 3,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-11",
        "opp": "@ WSH",
        "decision": null,
        "ip": "4.0",
        "er": 1,
        "h": 3,
        "bb": 4,
        "so": 5,
        "era": 2.25
      },
      {
        "date": "2026-09-16",
        "opp": "vs SEA",
        "decision": "L",
        "ip": "4.1",
        "er": 4,
        "h": 5,
        "bb": 5,
        "so": 4,
        "era": 8.31
      },
      {
        "date": "2026-09-22",
        "opp": "@ OAK",
        "decision": null,
        "ip": "5.1",
        "er": 2,
        "h": 7,
        "bb": 1,
        "so": 2,
        "era": 3.38
      }
    ],
    "home_pitcher": "Brady Basso",
    "home_pitcher_id": 669620,
    "home_pitcher_stats": {
      "wins": 1,
      "losses": 3,
      "era": "5.36"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-18",
        "opp": "@ KC",
        "decision": null,
        "ip": "2.1",
        "er": 0,
        "h": 2,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-08-23",
        "opp": "@ HOU",
        "decision": null,
        "ip": "1.0",
        "er": 3,
        "h": 4,
        "bb": 0,
        "so": 0,
        "era": 27.0
      },
      {
        "date": "2026-08-26",
        "opp": "vs MIN",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-28",
        "opp": "vs BAL",
        "decision": "L",
        "ip": "1.0",
        "er": 1,
        "h": 2,
        "bb": 1,
        "so": 2,
        "era": 9.0
      },
      {
        "date": "2026-08-30",
        "opp": "vs BAL",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-09-01",
        "opp": "@ TEX",
        "decision": "L",
        "ip": "2.1",
        "er": 2,
        "h": 4,
        "bb": 0,
        "so": 2,
        "era": 7.71
      },
      {
        "date": "2026-09-04",
        "opp": "@ SEA",
        "decision": "W",
        "ip": "3.0",
        "er": 0,
        "h": 3,
        "bb": 1,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-09-09",
        "opp": "vs TOR",
        "decision": null,
        "ip": "4.0",
        "er": 0,
        "h": 3,
        "bb": 2,
        "so": 4,
        "era": 0.0
      },
      {
        "date": "2026-09-16",
        "opp": "@ TB",
        "decision": "L",
        "ip": "2.2",
        "er": 4,
        "h": 7,
        "bb": 0,
        "so": 0,
        "era": 13.5
      },
      {
        "date": "2026-09-22",
        "opp": "vs LAA",
        "decision": null,
        "ip": "2.0",
        "er": 5,
        "h": 6,
        "bb": 2,
        "so": 2,
        "era": 22.5
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 48.5,
      "home": 51.5
    },
    "expected_score": {
      "away": 1.2,
      "home": 2.2
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "L",
      "home_handedness": "L",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 35.9,
        "sp_detail": {
          "era": 4.31,
          "whip": 1.48,
          "k9": 8.2,
          "avg_ip": 4.8,
          "qs_rate": 30.0,
          "last3_era": 3.04,
          "last_start_era": 3.38,
          "last2_eras": [
            8.31,
            3.38
          ],
          "recent_avg_era": 3.38,
          "recent_bad_start": false,
          "trend": "stable",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 35,
          "fb_velo": 95.5,
          "pitch_arsenal": [
            "FB",
            "SL",
            "FS",
            "CB",
            "CT"
          ],
          "secondary_pitches": [
            "SL",
            "FS",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 41.4,
              "velo": 95.5
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 25.2,
              "velo": 87.4
            },
            {
              "code": "FS",
              "abbr": "FS",
              "pct": 17.3,
              "velo": 86.6
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 10.3,
              "velo": 79.4
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 5.9,
              "velo": 90.2
            }
          ]
        },
        "bp_score": 33.1,
        "bp_detail": {
          "bullpen_era": 4.16,
          "recent_era": 6.65,
          "recent_appearances": 21,
          "recent_ip": 23.0,
          "team_era": 4.11,
          "bp_ip": 315.7,
          "bp_count": 9,
          "closer_era": 4.11,
          "closer_name": "Ben Joyce"
        },
        "bat_score": 17.1,
        "bat_detail": {
          "recent_avg": 0.222,
          "runs_per_g": 3.8,
          "hr_per_g": 0.7,
          "bb_per_g": 3.0,
          "season_ops": 0.646,
          "season_slg": null,
          "season_avg": 0.222,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Zach Neto(0.690)",
            "Mike Trout(0.804)",
            "Vaughn Grissom(0.718)",
            "Denzer Guzman(0.637)",
            "Moisés Ballesteros(0.632)",
            "Jose Siri(0.772)",
            "Wade Meckler(0.699)",
            "Christian Moore(0.456)",
            "Bryce Teodosio(0.402)"
          ],
          "lineup_players": [
            {
              "id": 687263,
              "name": "Zach Neto",
              "ops": 0.69,
              "avg": 0.167
            },
            {
              "id": 545361,
              "name": "Mike Trout",
              "ops": 0.804,
              "avg": 0.258
            },
            {
              "id": 687093,
              "name": "Vaughn Grissom",
              "ops": 0.718,
              "avg": 0.235
            },
            {
              "id": 694203,
              "name": "Denzer Guzman",
              "ops": 0.637,
              "avg": 0.237
            },
            {
              "id": 694208,
              "name": "Moisés Ballesteros",
              "ops": 0.632,
              "avg": 0.226
            },
            {
              "id": 642350,
              "name": "Jose Siri",
              "ops": 0.772,
              "avg": 0.323
            },
            {
              "id": 685133,
              "name": "Wade Meckler",
              "ops": 0.699,
              "avg": 0.278
            },
            {
              "id": 695681,
              "name": "Christian Moore",
              "ops": 0.456,
              "avg": 0.176
            },
            {
              "id": 669326,
              "name": "Bryce Teodosio",
              "ops": 0.402,
              "avg": 0.1
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 3.6,
          "last5_avg": 0.227,
          "home_split": {
            "recent_avg": 0.224,
            "runs_per_g": 3.7,
            "hr_per_g": 0.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.244,
            "runs_per_g": 4.0,
            "hr_per_g": 1.1,
            "n_games": 10
          }
        },
        "sit_score": 39.6,
        "total": 31.7
      },
      "home": {
        "sp_score": 29.6,
        "sp_detail": {
          "era": 6.33,
          "whip": 1.78,
          "k9": 5.1,
          "avg_ip": 2.1,
          "qs_rate": 0.0,
          "last3_era": 7.07,
          "last_start_era": 22.5,
          "last2_eras": [
            13.5,
            22.5
          ],
          "recent_avg_era": 22.5,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 28,
          "fb_velo": 92.5,
          "pitch_arsenal": [
            "FB",
            "CB",
            "SL",
            "CT",
            "SI",
            "CH"
          ],
          "secondary_pitches": [
            "CB",
            "SL",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 44.6,
              "velo": 92.5
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 17.7,
              "velo": 77.4
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 14.6,
              "velo": 83.6
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 10.9,
              "velo": 89.1
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 7.5,
              "velo": 92.4
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 4.1,
              "velo": 86.7
            }
          ]
        },
        "bp_score": 55.4,
        "bp_detail": {
          "bullpen_era": 4.4,
          "recent_era": 2.0,
          "recent_appearances": 22,
          "recent_ip": 29.0,
          "team_era": 5.31,
          "bp_ip": 343.7,
          "bp_count": 9,
          "closer_era": 3.42,
          "closer_name": "Hogan Harris"
        },
        "bat_score": 27.9,
        "bat_detail": {
          "recent_avg": 0.252,
          "runs_per_g": 3.8,
          "hr_per_g": 1.2,
          "bb_per_g": 3.0,
          "season_ops": 0.717,
          "season_slg": null,
          "season_avg": 0.252,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "L",
          "splits_used": false,
          "lineup_ops": [
            "Henry Bolte(0.702)",
            "Shea Langeliers(0.779)",
            "Lawrence Butler(0.940)",
            "Zack Gelof(0.799)",
            "Tommy White(0.655)",
            "Joshua Kuroda-Grauer(0.961)",
            "Jonah Heim(0.424)",
            "Alika Williams(0.810)",
            "Denzel Clarke(0.380)"
          ],
          "lineup_players": [
            {
              "id": 703607,
              "name": "Henry Bolte",
              "ops": 0.702,
              "avg": 0.222
            },
            {
              "id": 669127,
              "name": "Shea Langeliers",
              "ops": 0.779,
              "avg": 0.333
            },
            {
              "id": 671732,
              "name": "Lawrence Butler",
              "ops": 0.94,
              "avg": 0.324
            },
            {
              "id": 680869,
              "name": "Zack Gelof",
              "ops": 0.799,
              "avg": 0.263
            },
            {
              "id": 695720,
              "name": "Tommy White",
              "ops": 0.655,
              "avg": 0.28
            },
            {
              "id": 811965,
              "name": "Joshua Kuroda-Grauer",
              "ops": 0.961,
              "avg": 0.361
            },
            {
              "id": 641680,
              "name": "Jonah Heim",
              "ops": 0.424,
              "avg": 0.086
            },
            {
              "id": 675961,
              "name": "Alika Williams",
              "ops": 0.81,
              "avg": 0.32
            },
            {
              "id": 672016,
              "name": "Denzel Clarke",
              "ops": 0.38,
              "avg": 0.083
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 3.8,
          "last5_avg": 0.244,
          "home_split": {
            "recent_avg": 0.252,
            "runs_per_g": 4.5,
            "hr_per_g": 1.3,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.221,
            "runs_per_g": 3.4,
            "hr_per_g": 1.3,
            "n_games": 10
          }
        },
        "sit_score": 45.4,
        "total": 36.1
      }
    },
    "scores": {
      "away_offense": 17.1,
      "away_defense": 34.5,
      "home_offense": 27.9,
      "home_defense": 46.4
    },
    "actual_score": {
      "away": 7,
      "home": 9
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Athletics",
    "model_winner": "Athletics",
    "model_correct": true,
    "notes": "Los Angeles Angels IL: Anthony Rendon, George Klassen, Gustavo Campero, Jack Kochanowicz, Kyren Paris 외 다수 / Athletics IL: Brent Rooker, Brooks Kriske, Gage Jump, Gunnar Hoglund, J.T. Ginn 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 51.5% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "6:40 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 823089,
    "away": "Houston Astros",
    "home": "Seattle Mariners",
    "away_standing": {
      "div_rank": 1,
      "div_name": "AL West",
      "wins": 78,
      "losses": 79,
      "games_back": "-"
    },
    "home_standing": {
      "div_rank": 3,
      "div_name": "AL West",
      "wins": 73,
      "losses": 84,
      "games_back": "5.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "2-5",
          "home": true
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "6-2",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-6",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "3-6",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "2-4",
          "home": true
        }
      ],
      "wins": 1,
      "losses": 4,
      "streak": -3
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-15",
          "result": "L",
          "score": "1-2",
          "home": false
        },
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "7-2",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "5-4",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "3-5",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "2-1",
          "home": false
        }
      ],
      "wins": 3,
      "losses": 2,
      "streak": 1
    },
    "away_pitcher": "Cristian Javier",
    "away_pitcher_id": 664299,
    "away_pitcher_stats": {
      "wins": 2,
      "losses": 6,
      "era": "5.14"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-07-27",
        "opp": "@ LAA",
        "decision": "W",
        "ip": "5.0",
        "er": 0,
        "h": 3,
        "bb": 0,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-08-03",
        "opp": "vs TOR",
        "decision": "L",
        "ip": "6.0",
        "er": 3,
        "h": 7,
        "bb": 0,
        "so": 2,
        "era": 4.5
      },
      {
        "date": "2026-08-09",
        "opp": "@ SD",
        "decision": "L",
        "ip": "5.0",
        "er": 4,
        "h": 8,
        "bb": 1,
        "so": 2,
        "era": 7.2
      },
      {
        "date": "2026-08-18",
        "opp": "vs LAA",
        "decision": "L",
        "ip": "4.0",
        "er": 2,
        "h": 5,
        "bb": 1,
        "so": 5,
        "era": 4.5
      },
      {
        "date": "2026-08-23",
        "opp": "vs OAK",
        "decision": null,
        "ip": "4.0",
        "er": 3,
        "h": 4,
        "bb": 2,
        "so": 8,
        "era": 6.75
      },
      {
        "date": "2026-08-29",
        "opp": "@ NYM",
        "decision": "L",
        "ip": "4.1",
        "er": 3,
        "h": 6,
        "bb": 0,
        "so": 4,
        "era": 6.23
      },
      {
        "date": "2026-09-04",
        "opp": "vs OAK",
        "decision": "W",
        "ip": "5.0",
        "er": 1,
        "h": 1,
        "bb": 3,
        "so": 8,
        "era": 1.8
      },
      {
        "date": "2026-09-10",
        "opp": "@ PHI",
        "decision": null,
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 5,
        "era": 1.5
      },
      {
        "date": "2026-09-16",
        "opp": "vs KC",
        "decision": "L",
        "ip": "5.1",
        "er": 2,
        "h": 5,
        "bb": 3,
        "so": 2,
        "era": 3.38
      },
      {
        "date": "2026-09-22",
        "opp": "@ SEA",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 1,
        "bb": 2,
        "so": 2,
        "era": 0.0
      }
    ],
    "home_pitcher": "Logan Gilbert",
    "home_pitcher_id": 669302,
    "home_pitcher_stats": {
      "wins": 12,
      "losses": 11,
      "era": "3.81"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-01",
        "opp": "vs MIN",
        "decision": null,
        "ip": "7.0",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 6,
        "era": 3.86
      },
      {
        "date": "2026-08-07",
        "opp": "vs TB",
        "decision": "L",
        "ip": "6.2",
        "er": 2,
        "h": 4,
        "bb": 4,
        "so": 6,
        "era": 2.7
      },
      {
        "date": "2026-08-13",
        "opp": "@ NYY",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 1,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-08-19",
        "opp": "@ MIL",
        "decision": "W",
        "ip": "5.0",
        "er": 3,
        "h": 4,
        "bb": 4,
        "so": 5,
        "era": 5.4
      },
      {
        "date": "2026-08-24",
        "opp": "vs PHI",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 5,
        "bb": 2,
        "so": 5,
        "era": 3.6
      },
      {
        "date": "2026-08-30",
        "opp": "@ TOR",
        "decision": "L",
        "ip": "5.0",
        "er": 3,
        "h": 6,
        "bb": 1,
        "so": 4,
        "era": 5.4
      },
      {
        "date": "2026-09-04",
        "opp": "vs OAK",
        "decision": "L",
        "ip": "4.2",
        "er": 7,
        "h": 8,
        "bb": 2,
        "so": 10,
        "era": 13.5
      },
      {
        "date": "2026-09-10",
        "opp": "vs TEX",
        "decision": "W",
        "ip": "7.0",
        "er": 3,
        "h": 3,
        "bb": 2,
        "so": 11,
        "era": 3.86
      },
      {
        "date": "2026-09-15",
        "opp": "@ LAA",
        "decision": "L",
        "ip": "6.0",
        "er": 2,
        "h": 6,
        "bb": 2,
        "so": 7,
        "era": 3.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs HOU",
        "decision": "L",
        "ip": "4.0",
        "er": 4,
        "h": 10,
        "bb": 2,
        "so": 3,
        "era": 9.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 61.5,
      "home": 38.5
    },
    "expected_score": {
      "away": 2.7,
      "home": 1.8
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 47.0,
        "sp_detail": {
          "era": 3.66,
          "whip": 1.2,
          "k9": 8.7,
          "avg_ip": 4.7,
          "qs_rate": 20.0,
          "last3_era": 2.78,
          "last_start_era": 0.0,
          "last2_eras": [
            3.38,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 29,
          "fb_velo": 93.2,
          "pitch_arsenal": [
            "FB",
            "SW",
            "CH",
            "CT"
          ],
          "secondary_pitches": [
            "SW",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 46.4,
              "velo": 93.2
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 26.8,
              "velo": 81.3
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 16.1,
              "velo": 85.5
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 7.8,
              "velo": 89.0
            }
          ]
        },
        "bp_score": 58.9,
        "bp_detail": {
          "bullpen_era": 3.67,
          "recent_era": 4.74,
          "recent_appearances": 18,
          "recent_ip": 24.7,
          "team_era": 3.89,
          "bp_ip": 336.0,
          "bp_count": 8,
          "closer_era": 0.83,
          "closer_name": "Josh Hader"
        },
        "bat_score": 37.2,
        "bat_detail": {
          "recent_avg": 0.254,
          "runs_per_g": 3.3,
          "hr_per_g": 1.9,
          "bb_per_g": 3.0,
          "season_ops": 0.761,
          "season_slg": null,
          "season_avg": 0.254,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Jeremy Peña(0.867)",
            "Yordan Alvarez(1.117)",
            "Isaac Paredes(0.916)",
            "Jose Altuve(0.726)",
            "Yainer Diaz(0.880)",
            "Christian Walker(0.528)",
            "Taylor Trammell(0.623)",
            "Cam Smith(0.510)",
            "Brice Matthews(0.678)"
          ],
          "lineup_players": [
            {
              "id": 665161,
              "name": "Jeremy Peña",
              "ops": 0.867,
              "avg": 0.308
            },
            {
              "id": 670541,
              "name": "Yordan Alvarez",
              "ops": 1.117,
              "avg": 0.364
            },
            {
              "id": 670623,
              "name": "Isaac Paredes",
              "ops": 0.916,
              "avg": 0.286
            },
            {
              "id": 514888,
              "name": "Jose Altuve",
              "ops": 0.726,
              "avg": 0.3
            },
            {
              "id": 673237,
              "name": "Yainer Diaz",
              "ops": 0.88,
              "avg": 0.382
            },
            {
              "id": 572233,
              "name": "Christian Walker",
              "ops": 0.528,
              "avg": 0.111
            },
            {
              "id": 666211,
              "name": "Taylor Trammell",
              "ops": 0.623,
              "avg": 0.148
            },
            {
              "id": 701358,
              "name": "Cam Smith",
              "ops": 0.51,
              "avg": 0.111
            },
            {
              "id": 694728,
              "name": "Brice Matthews",
              "ops": 0.678,
              "avg": 0.273
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 4.0,
          "last5_avg": 0.264,
          "home_split": {
            "recent_avg": 0.233,
            "runs_per_g": 3.3,
            "hr_per_g": 1.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.233,
            "runs_per_g": 4.0,
            "hr_per_g": 1.6,
            "n_games": 10
          }
        },
        "sit_score": 60.0,
        "total": 52.7
      },
      "home": {
        "sp_score": 36.9,
        "sp_detail": {
          "era": 4.63,
          "whip": 1.35,
          "k9": 10.2,
          "avg_ip": 5.6,
          "qs_rate": 50.0,
          "last3_era": 6.41,
          "last_start_era": 9.0,
          "last2_eras": [
            3.0,
            9.0
          ],
          "recent_avg_era": 9.0,
          "recent_bad_start": true,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 7,
          "rest_note": "extra_rest",
          "age": 29,
          "fb_velo": 96.4,
          "pitch_arsenal": [
            "FB",
            "SL",
            "FS",
            "CH",
            "SW",
            "CT"
          ],
          "secondary_pitches": [
            "SL",
            "FS",
            "CH",
            "SW"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 40.3,
              "velo": 96.4
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 25.0,
              "velo": 86.8
            },
            {
              "code": "FS",
              "abbr": "FS",
              "pct": 14.9,
              "velo": 81.9
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 7.2,
              "velo": 85.8
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 7.1,
              "velo": 81.2
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 5.1,
              "velo": 91.2
            }
          ]
        },
        "bp_score": 43.7,
        "bp_detail": {
          "bullpen_era": 4.06,
          "recent_era": 4.5,
          "recent_appearances": 20,
          "recent_ip": 18.0,
          "team_era": 4.02,
          "bp_ip": 423.3,
          "bp_count": 9,
          "closer_era": 4.24,
          "closer_name": "Andrés Muñoz"
        },
        "bat_score": 26.4,
        "bat_detail": {
          "recent_avg": 0.229,
          "runs_per_g": 5.3,
          "hr_per_g": 1.4,
          "bb_per_g": 3.0,
          "season_ops": 0.715,
          "season_slg": null,
          "season_avg": 0.229,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "J.P. Crawford(0.698)",
            "Randy Arozarena(1.019)",
            "Dominic Canzone(0.919)",
            "Cal Raleigh(0.655)",
            "Julio Rodríguez(0.757)",
            "Josh Naylor(0.649)",
            "Cole Young(0.671)",
            "Lazaro Montes(0.692)",
            "Brock Rodden(0.376)"
          ],
          "lineup_players": [
            {
              "id": 641487,
              "name": "J.P. Crawford",
              "ops": 0.698,
              "avg": 0.233
            },
            {
              "id": 668227,
              "name": "Randy Arozarena",
              "ops": 1.019,
              "avg": 0.368
            },
            {
              "id": 686527,
              "name": "Dominic Canzone",
              "ops": 0.919,
              "avg": 0.342
            },
            {
              "id": 663728,
              "name": "Cal Raleigh",
              "ops": 0.655,
              "avg": 0.158
            },
            {
              "id": 677594,
              "name": "Julio Rodríguez",
              "ops": 0.757,
              "avg": 0.325
            },
            {
              "id": 647304,
              "name": "Josh Naylor",
              "ops": 0.649,
              "avg": 0.279
            },
            {
              "id": 702284,
              "name": "Cole Young",
              "ops": 0.671,
              "avg": 0.229
            },
            {
              "id": 703155,
              "name": "Lazaro Montes",
              "ops": 0.692,
              "avg": 0.13
            },
            {
              "id": 801126,
              "name": "Brock Rodden",
              "ops": 0.376,
              "avg": 0.0
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 3.4,
          "last5_avg": 0.211,
          "home_split": {
            "recent_avg": 0.198,
            "runs_per_g": 3.0,
            "hr_per_g": 0.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.285,
            "runs_per_g": 6.1,
            "hr_per_g": 1.4,
            "n_games": 10
          }
        },
        "sit_score": 47.5,
        "total": 38.4
      }
    },
    "scores": {
      "away_offense": 37.2,
      "away_defense": 53.0,
      "home_offense": 26.4,
      "home_defense": 40.3
    },
    "actual_score": {
      "away": 7,
      "home": 0
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Houston Astros",
    "model_winner": "Houston Astros",
    "model_correct": true,
    "notes": "Houston Astros IL: Brandon Walter, Carlos Correa, Cristian Javier, Enyel De Los Santos, Jake Meyers 외 다수 / Seattle Mariners IL: Brendan Donovan, Brennen Davis, Cole Wilcox, Colt Emerson, Emerson Hancock 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "6:45 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 823166,
    "away": "Minnesota Twins",
    "home": "San Francisco Giants",
    "away_standing": {
      "div_rank": 3,
      "div_name": "AL Central",
      "wins": 75,
      "losses": 84,
      "games_back": "8.0"
    },
    "home_standing": {
      "div_rank": 4,
      "div_name": "NL West",
      "wins": 65,
      "losses": 94,
      "games_back": "33.0"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-17",
          "result": "L",
          "score": "4-5",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "3-0",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "5-6",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "8-0",
          "home": false
        },
        {
          "date": "2026-09-21",
          "result": "L",
          "score": "2-5",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -1
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "6-5",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "2-8",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "4-10",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "1-3",
          "home": false
        },
        {
          "date": "2026-09-21",
          "result": "W",
          "score": "5-2",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": 1
    },
    "away_pitcher": "Taj Bradley",
    "away_pitcher_id": 671737,
    "away_pitcher_stats": {
      "wins": 12,
      "losses": 6,
      "era": "3.73"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-02",
        "opp": "@ SEA",
        "decision": null,
        "ip": "6.0",
        "er": 3,
        "h": 7,
        "bb": 4,
        "so": 2,
        "era": 4.5
      },
      {
        "date": "2026-08-08",
        "opp": "@ MIL",
        "decision": null,
        "ip": "5.0",
        "er": 3,
        "h": 5,
        "bb": 2,
        "so": 7,
        "era": 5.4
      },
      {
        "date": "2026-08-13",
        "opp": "vs PHI",
        "decision": "L",
        "ip": "4.0",
        "er": 5,
        "h": 7,
        "bb": 4,
        "so": 3,
        "era": 11.25
      },
      {
        "date": "2026-08-19",
        "opp": "vs ATL",
        "decision": null,
        "ip": "5.0",
        "er": 1,
        "h": 2,
        "bb": 3,
        "so": 7,
        "era": 1.8
      },
      {
        "date": "2026-08-25",
        "opp": "@ OAK",
        "decision": "L",
        "ip": "6.2",
        "er": 4,
        "h": 7,
        "bb": 1,
        "so": 11,
        "era": 5.4
      },
      {
        "date": "2026-08-31",
        "opp": "vs DET",
        "decision": "W",
        "ip": "7.0",
        "er": 1,
        "h": 4,
        "bb": 3,
        "so": 9,
        "era": 1.29
      },
      {
        "date": "2026-09-05",
        "opp": "@ CWS",
        "decision": "W",
        "ip": "5.0",
        "er": 4,
        "h": 4,
        "bb": 2,
        "so": 6,
        "era": 7.2
      },
      {
        "date": "2026-09-11",
        "opp": "vs CLE",
        "decision": null,
        "ip": "6.0",
        "er": 0,
        "h": 1,
        "bb": 4,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-17",
        "opp": "@ LAA",
        "decision": null,
        "ip": "7.0",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 9,
        "era": 3.86
      },
      {
        "date": "2026-09-22",
        "opp": "@ SF",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 3,
        "so": 6,
        "era": 1.5
      }
    ],
    "home_pitcher": "Anthony Molina",
    "home_pitcher_id": 683627,
    "home_pitcher_stats": {
      "wins": 3,
      "losses": 2,
      "era": "3.97"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-05-01",
        "opp": "@ COL",
        "decision": null,
        "ip": "2.0",
        "er": 0,
        "h": 0,
        "bb": 1,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-06-14",
        "opp": "@ NYM",
        "decision": null,
        "ip": "1.0",
        "er": 1,
        "h": 3,
        "bb": 0,
        "so": 2,
        "era": 9.0
      },
      {
        "date": "2026-06-16",
        "opp": "vs SF",
        "decision": null,
        "ip": "2.0",
        "er": 2,
        "h": 3,
        "bb": 0,
        "so": 0,
        "era": 9.0
      },
      {
        "date": "2026-08-25",
        "opp": "vs CIN",
        "decision": "W",
        "ip": "4.1",
        "er": 0,
        "h": 1,
        "bb": 1,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-08-31",
        "opp": "@ ATL",
        "decision": "W",
        "ip": "5.0",
        "er": 3,
        "h": 6,
        "bb": 0,
        "so": 6,
        "era": 5.4
      },
      {
        "date": "2026-09-05",
        "opp": "@ NYM",
        "decision": "W",
        "ip": "5.0",
        "er": 0,
        "h": 1,
        "bb": 1,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-11",
        "opp": "vs SD",
        "decision": "L",
        "ip": "3.0",
        "er": 7,
        "h": 9,
        "bb": 2,
        "so": 4,
        "era": 21.0
      },
      {
        "date": "2026-09-16",
        "opp": "@ STL",
        "decision": null,
        "ip": "5.2",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs MIN",
        "decision": "L",
        "ip": "6.0",
        "er": 2,
        "h": 4,
        "bb": 1,
        "so": 6,
        "era": 3.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 60.6,
      "home": 39.4
    },
    "expected_score": {
      "away": 1.8,
      "home": 1.0
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.25,
        "bat": 0.3,
        "sit": 0.15
      },
      "away": {
        "sp_score": 64.9,
        "sp_detail": {
          "era": 3.9,
          "whip": 1.25,
          "k9": 10.3,
          "avg_ip": 5.8,
          "qs_rate": 50.0,
          "last3_era": 2.61,
          "last_start_era": 1.5,
          "last2_eras": [
            3.86,
            1.5
          ],
          "recent_avg_era": 1.5,
          "recent_bad_start": false,
          "trend": "hot",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 5,
          "rest_note": null,
          "age": 25,
          "fb_velo": 97.2,
          "pitch_arsenal": [
            "FB",
            "CT",
            "FS",
            "CB"
          ],
          "secondary_pitches": [
            "FS",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 46.6,
              "velo": 97.2
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 24.5,
              "velo": 89.7
            },
            {
              "code": "FS",
              "abbr": "FS",
              "pct": 19.6,
              "velo": 91.5
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 9.3,
              "velo": 82.8
            }
          ]
        },
        "bp_score": 50.8,
        "bp_detail": {
          "bullpen_era": 3.99,
          "recent_era": 2.36,
          "recent_appearances": 28,
          "recent_ip": 26.7,
          "team_era": 4.21,
          "bp_ip": 473.3,
          "bp_count": 8,
          "closer_era": 4.36,
          "closer_name": "Yoendrys Gómez"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.24,
          "runs_per_g": 3.8,
          "hr_per_g": 1.0,
          "bb_per_g": 3.0,
          "season_ops": 0.676,
          "season_slg": null,
          "season_avg": 0.24,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Luke Keaschall(0.813)",
            "Brooks Lee(0.812)",
            "Kody Clemens(0.547)",
            "Josh Bell(0.681)",
            "Walker Jenkins(0.839)",
            "Emmanuel Rodriguez(0.666)",
            "Royce Lewis(0.535)",
            "Victor Caratini(0.512)",
            "Ryan Kreidler(0.681)"
          ],
          "lineup_players": [
            {
              "id": 807712,
              "name": "Luke Keaschall",
              "ops": 0.813,
              "avg": 0.333
            },
            {
              "id": 686797,
              "name": "Brooks Lee",
              "ops": 0.812,
              "avg": 0.368
            },
            {
              "id": 665019,
              "name": "Kody Clemens",
              "ops": 0.547,
              "avg": 0.118
            },
            {
              "id": 605137,
              "name": "Josh Bell",
              "ops": 0.681,
              "avg": 0.262
            },
            {
              "id": 805805,
              "name": "Walker Jenkins",
              "ops": 0.839,
              "avg": 0.343
            },
            {
              "id": 691181,
              "name": "Emmanuel Rodriguez",
              "ops": 0.666,
              "avg": 0.182
            },
            {
              "id": 668904,
              "name": "Royce Lewis",
              "ops": 0.535,
              "avg": 0.179
            },
            {
              "id": 605170,
              "name": "Victor Caratini",
              "ops": 0.512,
              "avg": 0.2
            },
            {
              "id": 668952,
              "name": "Ryan Kreidler",
              "ops": 0.681,
              "avg": 0.174
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 4.4,
          "last5_avg": 0.24,
          "home_split": {
            "recent_avg": 0.24,
            "runs_per_g": 5.4,
            "hr_per_g": 1.2,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.234,
            "runs_per_g": 3.8,
            "hr_per_g": 1.2,
            "n_games": 10
          }
        },
        "sit_score": 51.6,
        "total": 52.6
      },
      "home": {
        "sp_score": 39.4,
        "sp_detail": {
          "era": 3.97,
          "whip": 1.06,
          "k9": 8.2,
          "avg_ip": 3.8,
          "qs_rate": 11.1,
          "last3_era": 4.38,
          "last_start_era": 3.0,
          "last2_eras": [
            0.0,
            3.0
          ],
          "recent_avg_era": 3.0,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 9,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 24,
          "fb_velo": 96.3,
          "pitch_arsenal": [
            "FB",
            "CH",
            "CB",
            "SL",
            "SV"
          ],
          "secondary_pitches": [
            "CH",
            "CB",
            "SL",
            "SV"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 48.4,
              "velo": 96.3
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 21.6,
              "velo": 87.0
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 15.2,
              "velo": 84.4
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 10.5,
              "velo": 87.9
            },
            {
              "code": "SV",
              "abbr": "SV",
              "pct": 4.1,
              "velo": 84.5
            }
          ]
        },
        "bp_score": 34.9,
        "bp_detail": {
          "bullpen_era": 4.14,
          "recent_era": 4.85,
          "recent_appearances": 26,
          "recent_ip": 39.0,
          "team_era": 4.06,
          "bp_ip": 261.0,
          "bp_count": 11,
          "closer_era": 4.6,
          "closer_name": "Joel Kuhnel"
        },
        "bat_score": 15.0,
        "bat_detail": {
          "recent_avg": 0.218,
          "runs_per_g": 3.7,
          "hr_per_g": 0.7,
          "bb_per_g": 3.0,
          "season_ops": 0.605,
          "season_slg": null,
          "season_avg": 0.218,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Drew Gilbert(0.911)",
            "Jonah Cox(0.627)",
            "Victor Bericoto(0.604)",
            "Turner Hill(0.544)",
            "Bo Davidson(0.664)",
            "Shay Whitcomb(0.552)",
            "Scott Bandura(0.465)",
            "Drew Cavanaugh(0.536)",
            "Christian Koss(0.538)"
          ],
          "lineup_players": [
            {
              "id": 687551,
              "name": "Drew Gilbert",
              "ops": 0.911,
              "avg": 0.333
            },
            {
              "id": 813841,
              "name": "Jonah Cox",
              "ops": 0.627,
              "avg": 0.216
            },
            {
              "id": 682674,
              "name": "Victor Bericoto",
              "ops": 0.604,
              "avg": 0.172
            },
            {
              "id": 806367,
              "name": "Turner Hill",
              "ops": 0.544,
              "avg": 0.152
            },
            {
              "id": 815589,
              "name": "Bo Davidson",
              "ops": 0.664,
              "avg": 0.3
            },
            {
              "id": 694376,
              "name": "Shay Whitcomb",
              "ops": 0.552,
              "avg": 0.267
            },
            {
              "id": 814154,
              "name": "Scott Bandura",
              "ops": 0.465,
              "avg": 0.19
            },
            {
              "id": 701852,
              "name": "Drew Cavanaugh",
              "ops": 0.536,
              "avg": 0.105
            },
            {
              "id": 683766,
              "name": "Christian Koss",
              "ops": 0.538,
              "avg": 0.229
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.8,
          "last5_avg": 0.196,
          "home_split": {
            "recent_avg": 0.24,
            "runs_per_g": 4.5,
            "hr_per_g": 0.9,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.223,
            "runs_per_g": 4.3,
            "hr_per_g": 1.2,
            "n_games": 10
          }
        },
        "sit_score": 42.2,
        "total": 34.5
      }
    },
    "scores": {
      "away_offense": 25.0,
      "away_defense": 57.9,
      "home_offense": 15.0,
      "home_defense": 37.1
    },
    "actual_score": {
      "away": 3,
      "home": 2
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Minnesota Twins",
    "model_winner": "Minnesota Twins",
    "model_correct": true,
    "notes": "Minnesota Twins IL: Anthony Banda, Byron Buxton, Cole Sands, David Festa, Kaelen Culpepper 외 다수 / San Francisco Giants IL: Adrian Houser, Blade Tidwell, Bryce Eldridge, Carson Whisenhunt, Casey Schmitt 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": false,
    "low_confidence_reason": null,
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "7:10 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 823897,
    "away": "San Diego Padres",
    "home": "Los Angeles Dodgers",
    "away_standing": {
      "div_rank": 2,
      "div_name": "NL West",
      "wins": 87,
      "losses": 70,
      "games_back": "10.0"
    },
    "home_standing": {
      "div_rank": 1,
      "div_name": "NL West",
      "wins": 97,
      "losses": 60,
      "games_back": "-"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "9-3",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "9-2",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "8-2",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "7-6",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "7-3",
          "home": true
        }
      ],
      "wins": 5,
      "losses": 0,
      "streak": 5
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "L",
          "score": "2-6",
          "home": false
        },
        {
          "date": "2026-09-17",
          "result": "W",
          "score": "8-2",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "W",
          "score": "8-2",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "W",
          "score": "10-4",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "3-1",
          "home": true
        }
      ],
      "wins": 4,
      "losses": 1,
      "streak": 4
    },
    "away_pitcher": "Michael King",
    "away_pitcher_id": 650633,
    "away_pitcher_stats": {
      "wins": 12,
      "losses": 10,
      "era": "3.21"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-03",
        "opp": "@ OAK",
        "decision": "L",
        "ip": "5.0",
        "er": 3,
        "h": 5,
        "bb": 4,
        "so": 5,
        "era": 5.4
      },
      {
        "date": "2026-08-08",
        "opp": "vs HOU",
        "decision": "W",
        "ip": "6.0",
        "er": 1,
        "h": 3,
        "bb": 1,
        "so": 6,
        "era": 1.5
      },
      {
        "date": "2026-08-14",
        "opp": "@ CLE",
        "decision": "W",
        "ip": "6.0",
        "er": 3,
        "h": 8,
        "bb": 1,
        "so": 4,
        "era": 4.5
      },
      {
        "date": "2026-08-19",
        "opp": "@ NYM",
        "decision": "L",
        "ip": "6.2",
        "er": 2,
        "h": 4,
        "bb": 2,
        "so": 4,
        "era": 2.7
      },
      {
        "date": "2026-08-25",
        "opp": "vs PIT",
        "decision": null,
        "ip": "7.0",
        "er": 0,
        "h": 5,
        "bb": 2,
        "so": 7,
        "era": 0.0
      },
      {
        "date": "2026-08-31",
        "opp": "@ CIN",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 3,
        "so": 6,
        "era": 0.0
      },
      {
        "date": "2026-09-06",
        "opp": "vs NYY",
        "decision": "W",
        "ip": "6.0",
        "er": 0,
        "h": 4,
        "bb": 3,
        "so": 3,
        "era": 0.0
      },
      {
        "date": "2026-09-12",
        "opp": "@ SF",
        "decision": "W",
        "ip": "7.0",
        "er": 3,
        "h": 5,
        "bb": 0,
        "so": 4,
        "era": 3.86
      },
      {
        "date": "2026-09-17",
        "opp": "@ COL",
        "decision": "W",
        "ip": "6.0",
        "er": 2,
        "h": 7,
        "bb": 2,
        "so": 6,
        "era": 3.0
      },
      {
        "date": "2026-09-22",
        "opp": "@ LAD",
        "decision": "L",
        "ip": "4.0",
        "er": 5,
        "h": 6,
        "bb": 2,
        "so": 5,
        "era": 11.25
      }
    ],
    "home_pitcher": "Brock Stewart",
    "home_pitcher_id": 592779,
    "home_pitcher_stats": {
      "wins": 1,
      "losses": 2,
      "era": "2.45"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-08-26",
        "opp": "@ ATL",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 2,
        "bb": 1,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-08-28",
        "opp": "@ DET",
        "decision": null,
        "ip": "0.2",
        "er": 0,
        "h": 1,
        "bb": 2,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-09-02",
        "opp": "vs STL",
        "decision": "L",
        "ip": "1.0",
        "er": 2,
        "h": 3,
        "bb": 0,
        "so": 1,
        "era": 18.0
      },
      {
        "date": "2026-09-03",
        "opp": "vs STL",
        "decision": "W",
        "ip": "0.2",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-09-06",
        "opp": "vs WSH",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 0,
        "bb": 1,
        "so": 0,
        "era": 0.0
      },
      {
        "date": "2026-09-11",
        "opp": "@ MIA",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-09-13",
        "opp": "@ MIA",
        "decision": null,
        "ip": "0.0",
        "er": 1,
        "h": 1,
        "bb": 0,
        "so": 0,
        "era": null
      },
      {
        "date": "2026-09-15",
        "opp": "@ CIN",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 1,
        "era": 0.0
      },
      {
        "date": "2026-09-20",
        "opp": "vs SF",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 0,
        "bb": 0,
        "so": 2,
        "era": 0.0
      },
      {
        "date": "2026-09-22",
        "opp": "vs SD",
        "decision": null,
        "ip": "1.0",
        "er": 0,
        "h": 1,
        "bb": 0,
        "so": 1,
        "era": 0.0
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 50.0,
      "home": 50.0
    },
    "expected_score": {
      "away": 3.8,
      "home": 3.5
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "lineup",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": false,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.2,
        "bp": 0.46,
        "bat": 0.19,
        "sit": 0.15
      },
      "away": {
        "sp_score": 51.7,
        "sp_detail": {
          "era": 2.87,
          "whip": 1.19,
          "k9": 7.5,
          "avg_ip": 6.0,
          "qs_rate": 80.0,
          "last3_era": 3.1,
          "last_start_era": 11.25,
          "last2_eras": [
            3.0,
            11.25
          ],
          "recent_avg_era": 11.25,
          "recent_bad_start": true,
          "trend": "stable",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 5,
          "rest_note": null,
          "age": 31,
          "fb_velo": 94.3,
          "pitch_arsenal": [
            "SI",
            "CH",
            "FB",
            "SW",
            "SL"
          ],
          "secondary_pitches": [
            "CH",
            "SW",
            "SL"
          ],
          "pitches_detail": [
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 27.9,
              "velo": 93.2
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 26.4,
              "velo": 86.5
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 20.3,
              "velo": 94.3
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 18.8,
              "velo": 82.0
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 6.6,
              "velo": 85.5
            }
          ]
        },
        "bp_score": 75.0,
        "bp_detail": {
          "bullpen_era": 2.69,
          "recent_era": 2.0,
          "recent_appearances": 13,
          "recent_ip": 13.0,
          "team_era": 3.6,
          "bp_ip": 375.0,
          "bp_count": 6,
          "closer_era": 1.21,
          "closer_name": "Mason Miller"
        },
        "bat_score": 54.0,
        "bat_detail": {
          "recent_avg": 0.323,
          "runs_per_g": 6.4,
          "hr_per_g": 1.8,
          "bb_per_g": 3.0,
          "season_ops": 0.82,
          "season_slg": null,
          "season_avg": 0.323,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Fernando Tatis Jr.(0.936)",
            "Dustin Harris(0.761)",
            "Manny Machado(0.634)",
            "Ty France(0.947)",
            "Jackson Merrill(0.828)",
            "Austin Hays(0.889)",
            "Xander Bogaerts(0.851)",
            "Luis Campusano(0.706)",
            "Jake Cronenworth(0.831)"
          ],
          "lineup_players": [
            {
              "id": 665487,
              "name": "Fernando Tatis Jr.",
              "ops": 0.936,
              "avg": 0.293
            },
            {
              "id": 687957,
              "name": "Dustin Harris",
              "ops": 0.761,
              "avg": 0.325
            },
            {
              "id": 592518,
              "name": "Manny Machado",
              "ops": 0.634,
              "avg": 0.154
            },
            {
              "id": 664034,
              "name": "Ty France",
              "ops": 0.947,
              "avg": 0.405
            },
            {
              "id": 701538,
              "name": "Jackson Merrill",
              "ops": 0.828,
              "avg": 0.333
            },
            {
              "id": 669720,
              "name": "Austin Hays",
              "ops": 0.889,
              "avg": 0.421
            },
            {
              "id": 593428,
              "name": "Xander Bogaerts",
              "ops": 0.851,
              "avg": 0.368
            },
            {
              "id": 669134,
              "name": "Luis Campusano",
              "ops": 0.706,
              "avg": 0.22
            },
            {
              "id": 630105,
              "name": "Jake Cronenworth",
              "ops": 0.831,
              "avg": 0.385
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 6.2,
          "last5_avg": 0.296,
          "home_split": {
            "recent_avg": 0.267,
            "runs_per_g": 5.0,
            "hr_per_g": 1.5,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.291,
            "runs_per_g": 5.5,
            "hr_per_g": 1.4,
            "n_games": 10
          }
        },
        "sit_score": 52.8,
        "total": 63.0
      },
      "home": {
        "sp_score": 18.0,
        "sp_detail": {
          "era": 3.24,
          "whip": 1.68,
          "k9": 6.5,
          "avg_ip": 0.8,
          "qs_rate": 0.0,
          "last3_era": 2.25,
          "last_start_era": 0.0,
          "last2_eras": [
            0.0,
            0.0
          ],
          "recent_avg_era": 0.0,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 2,
          "rest_note": "short_rest",
          "age": 34,
          "fb_velo": 95.9,
          "pitch_arsenal": [
            "FB",
            "SW",
            "CH",
            "SI",
            "CT"
          ],
          "secondary_pitches": [
            "SW",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 36.8,
              "velo": 95.9
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 25.0,
              "velo": 83.9
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 15.1,
              "velo": 89.3
            },
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 14.6,
              "velo": 94.6
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 8.5,
              "velo": 92.2
            }
          ]
        },
        "bp_score": 69.4,
        "bp_detail": {
          "bullpen_era": 3.39,
          "recent_era": 2.16,
          "recent_appearances": 17,
          "recent_ip": 16.7,
          "team_era": 3.31,
          "bp_ip": 273.3,
          "bp_count": 7,
          "closer_era": 2.77,
          "closer_name": "Tanner Scott"
        },
        "bat_score": 53.9,
        "bat_detail": {
          "recent_avg": 0.266,
          "runs_per_g": 5.3,
          "hr_per_g": 1.8,
          "bb_per_g": 3.0,
          "season_ops": 0.827,
          "season_slg": null,
          "season_avg": 0.266,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Mookie Betts(0.976)",
            "Freddie Freeman(0.627)",
            "Will Smith(0.911)",
            "Max Muncy(0.769)",
            "Kyle Tucker(0.909)",
            "Teoscar Hernández(0.729)",
            "Josue De Paula(1.125)",
            "Andy Pages(0.801)",
            "Tommy Edman(0.595)"
          ],
          "lineup_players": [
            {
              "id": 605141,
              "name": "Mookie Betts",
              "ops": 0.976,
              "avg": 0.359
            },
            {
              "id": 518692,
              "name": "Freddie Freeman",
              "ops": 0.627,
              "avg": 0.214
            },
            {
              "id": 669257,
              "name": "Will Smith",
              "ops": 0.911,
              "avg": 0.303
            },
            {
              "id": 571970,
              "name": "Max Muncy",
              "ops": 0.769,
              "avg": 0.222
            },
            {
              "id": 663656,
              "name": "Kyle Tucker",
              "ops": 0.909,
              "avg": 0.312
            },
            {
              "id": 606192,
              "name": "Teoscar Hernández",
              "ops": 0.729,
              "avg": 0.231
            },
            {
              "id": 800543,
              "name": "Josue De Paula",
              "ops": 1.125,
              "avg": 0.357
            },
            {
              "id": 681624,
              "name": "Andy Pages",
              "ops": 0.801,
              "avg": 0.278
            },
            {
              "id": 669242,
              "name": "Tommy Edman",
              "ops": 0.595,
              "avg": 0.121
            }
          ],
          "bat_trend": "hot",
          "last5_rpg": 7.2,
          "last5_avg": 0.333,
          "home_split": {
            "recent_avg": 0.281,
            "runs_per_g": 6.9,
            "hr_per_g": 1.8,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.23,
            "runs_per_g": 4.0,
            "hr_per_g": 1.1,
            "n_games": 10
          }
        },
        "sit_score": 71.9,
        "total": 56.5
      }
    },
    "scores": {
      "away_offense": 54.0,
      "away_defense": 63.4,
      "home_offense": 53.9,
      "home_defense": 43.7
    },
    "actual_score": {
      "away": 0,
      "home": 7
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": "Los Angeles Dodgers",
    "model_winner": "Los Angeles Dodgers",
    "model_correct": true,
    "notes": "San Diego Padres IL: Bryan Hoeing, Gavin Sheets, Jason Adam, Jeremiah Estrada, Joe Musgrove 외 다수 / Los Angeles Dodgers IL: Ben Casparius, Brusdar Graterol, Dalton Rushing, Edgardo Henriquez, Eric Lauer 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 50.0% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": true
    },
    "model_version": "v15"
  },
  {
    "date": "2026-09-22",
    "status": "Final",
    "game_time": "3:35 PM PT",
    "game_number": 1,
    "doubleheader": false,
    "game_pk": 824785,
    "away": "Toronto Blue Jays",
    "home": "Baltimore Orioles",
    "away_standing": {
      "div_rank": 4,
      "div_name": "AL East",
      "wins": 77,
      "losses": 81,
      "games_back": "19.5"
    },
    "home_standing": {
      "div_rank": 5,
      "div_name": "AL East",
      "wins": 77,
      "losses": 81,
      "games_back": "19.5"
    },
    "away_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "5-1",
          "home": true
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "1-7",
          "home": false
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "2-6",
          "home": false
        },
        {
          "date": "2026-09-20",
          "result": "W",
          "score": "7-2",
          "home": false
        },
        {
          "date": "2026-09-21",
          "result": "L",
          "score": "3-4",
          "home": false
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": -1
    },
    "home_recent_form": {
      "games": [
        {
          "date": "2026-09-16",
          "result": "W",
          "score": "7-1",
          "home": false
        },
        {
          "date": "2026-09-18",
          "result": "L",
          "score": "5-6",
          "home": true
        },
        {
          "date": "2026-09-19",
          "result": "L",
          "score": "0-1",
          "home": true
        },
        {
          "date": "2026-09-20",
          "result": "L",
          "score": "0-3",
          "home": true
        },
        {
          "date": "2026-09-21",
          "result": "W",
          "score": "4-3",
          "home": true
        }
      ],
      "wins": 2,
      "losses": 3,
      "streak": 1
    },
    "away_pitcher": "Max Scherzer",
    "away_pitcher_id": 453286,
    "away_pitcher_stats": {
      "wins": 3,
      "losses": 9,
      "era": "6.14"
    },
    "away_pitcher_gamelog": [
      {
        "date": "2026-08-02",
        "opp": "vs STL",
        "decision": null,
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 5,
        "era": 1.5
      },
      {
        "date": "2026-08-08",
        "opp": "@ PHI",
        "decision": null,
        "ip": "5.1",
        "er": 2,
        "h": 5,
        "bb": 0,
        "so": 4,
        "era": 3.38
      },
      {
        "date": "2026-08-13",
        "opp": "vs BOS",
        "decision": "L",
        "ip": "5.0",
        "er": 1,
        "h": 3,
        "bb": 2,
        "so": 4,
        "era": 1.8
      },
      {
        "date": "2026-08-19",
        "opp": "@ TB",
        "decision": "L",
        "ip": "4.0",
        "er": 7,
        "h": 5,
        "bb": 3,
        "so": 1,
        "era": 15.75
      },
      {
        "date": "2026-08-25",
        "opp": "vs KC",
        "decision": "L",
        "ip": "5.0",
        "er": 2,
        "h": 4,
        "bb": 5,
        "so": 4,
        "era": 3.6
      },
      {
        "date": "2026-08-30",
        "opp": "vs SEA",
        "decision": "W",
        "ip": "7.0",
        "er": 0,
        "h": 1,
        "bb": 1,
        "so": 10,
        "era": 0.0
      },
      {
        "date": "2026-09-05",
        "opp": "@ KC",
        "decision": null,
        "ip": "5.2",
        "er": 3,
        "h": 5,
        "bb": 1,
        "so": 2,
        "era": 4.76
      },
      {
        "date": "2026-09-11",
        "opp": "vs BAL",
        "decision": "L",
        "ip": "5.0",
        "er": 6,
        "h": 7,
        "bb": 1,
        "so": 3,
        "era": 10.8
      },
      {
        "date": "2026-09-16",
        "opp": "vs DET",
        "decision": "W",
        "ip": "5.0",
        "er": 1,
        "h": 3,
        "bb": 3,
        "so": 3,
        "era": 1.8
      },
      {
        "date": "2026-09-23",
        "opp": "@ BAL",
        "decision": "L",
        "ip": "5.0",
        "er": 4,
        "h": 4,
        "bb": 3,
        "so": 7,
        "era": 7.2
      }
    ],
    "home_pitcher": "Chris Bassitt",
    "home_pitcher_id": 605135,
    "home_pitcher_stats": {
      "wins": 9,
      "losses": 5,
      "era": "4.59"
    },
    "home_pitcher_gamelog": [
      {
        "date": "2026-05-28",
        "opp": "vs TOR",
        "decision": null,
        "ip": "6.0",
        "er": 1,
        "h": 4,
        "bb": 1,
        "so": 2,
        "era": 1.5
      },
      {
        "date": "2026-06-03",
        "opp": "@ BOS",
        "decision": "L",
        "ip": "3.0",
        "er": 3,
        "h": 6,
        "bb": 2,
        "so": 1,
        "era": 9.0
      },
      {
        "date": "2026-08-14",
        "opp": "@ TB",
        "decision": null,
        "ip": "5.1",
        "er": 2,
        "h": 5,
        "bb": 3,
        "so": 3,
        "era": 3.38
      },
      {
        "date": "2026-08-19",
        "opp": "vs NYY",
        "decision": null,
        "ip": "5.2",
        "er": 3,
        "h": 8,
        "bb": 1,
        "so": 6,
        "era": 4.76
      },
      {
        "date": "2026-08-25",
        "opp": "@ STL",
        "decision": "W",
        "ip": "6.2",
        "er": 1,
        "h": 6,
        "bb": 0,
        "so": 5,
        "era": 1.35
      },
      {
        "date": "2026-08-30",
        "opp": "@ OAK",
        "decision": "W",
        "ip": "5.2",
        "er": 5,
        "h": 8,
        "bb": 3,
        "so": 4,
        "era": 7.94
      },
      {
        "date": "2026-09-05",
        "opp": "vs BOS",
        "decision": "L",
        "ip": "5.2",
        "er": 3,
        "h": 5,
        "bb": 4,
        "so": 6,
        "era": 4.76
      },
      {
        "date": "2026-09-11",
        "opp": "@ TOR",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 6,
        "bb": 3,
        "so": 5,
        "era": 3.6
      },
      {
        "date": "2026-09-16",
        "opp": "@ NYM",
        "decision": "W",
        "ip": "6.2",
        "er": 1,
        "h": 5,
        "bb": 1,
        "so": 6,
        "era": 1.35
      },
      {
        "date": "2026-09-23",
        "opp": "vs TOR",
        "decision": "W",
        "ip": "5.0",
        "er": 2,
        "h": 7,
        "bb": 1,
        "so": 2,
        "era": 3.6
      }
    ],
    "pred_model": "scorecard",
    "win_prob": {
      "away": 45.1,
      "home": 54.9
    },
    "expected_score": {
      "away": 1.7,
      "home": 1.8
    },
    "blend_detail": null,
    "ml_mc_agree": null,
    "ml_mc_conflict_level": null,
    "scorecard": {
      "bat_source": "prev_day",
      "away_handedness": "R",
      "home_handedness": "R",
      "bullpen_game": true,
      "any_cold_sp": true,
      "eff_weights": {
        "sp": 0.38,
        "bp": 0.3,
        "bat": 0.25,
        "sit": 0.15
      },
      "away": {
        "sp_score": 29.6,
        "sp_detail": {
          "era": 4.58,
          "whip": 1.15,
          "k9": 7.3,
          "avg_ip": 5.3,
          "qs_rate": 20.0,
          "last3_era": 4.55,
          "last_start_era": 7.2,
          "last2_eras": [
            1.8,
            7.2
          ],
          "recent_avg_era": 7.2,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 42,
          "fb_velo": 93.0,
          "pitch_arsenal": [
            "FB",
            "SL",
            "CH",
            "CB",
            "CT"
          ],
          "secondary_pitches": [
            "SL",
            "CH",
            "CB"
          ],
          "pitches_detail": [
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 47.6,
              "velo": 93.0
            },
            {
              "code": "SL",
              "abbr": "SL",
              "pct": 17.1,
              "velo": 85.4
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 15.0,
              "velo": 83.9
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 14.6,
              "velo": 74.6
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 5.7,
              "velo": 87.8
            }
          ]
        },
        "bp_score": 69.9,
        "bp_detail": {
          "bullpen_era": 3.26,
          "recent_era": 2.91,
          "recent_appearances": 18,
          "recent_ip": 21.7,
          "team_era": 3.59,
          "bp_ip": 325.7,
          "bp_count": 9,
          "closer_era": 1.29,
          "closer_name": "Louis Varland"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.223,
          "runs_per_g": 3.4,
          "hr_per_g": 0.5,
          "bb_per_g": 3.0,
          "season_ops": 0.664,
          "season_slg": null,
          "season_avg": 0.223,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Brett Bateman(0.845)",
            "Alejandro Kirk(0.790)",
            "Nathan Lukes(0.704)",
            "George Springer(0.734)",
            "Sean Keys(0.623)",
            "Kazuma Okamoto(0.707)",
            "Josh Smith(0.451)",
            "Andrés Giménez(0.623)",
            "Ernie Clement(0.501)"
          ],
          "lineup_players": [
            {
              "id": 703520,
              "name": "Brett Bateman",
              "ops": 0.845,
              "avg": 0.297
            },
            {
              "id": 672386,
              "name": "Alejandro Kirk",
              "ops": 0.79,
              "avg": 0.312
            },
            {
              "id": 664770,
              "name": "Nathan Lukes",
              "ops": 0.704,
              "avg": 0.296
            },
            {
              "id": 543807,
              "name": "George Springer",
              "ops": 0.734,
              "avg": 0.278
            },
            {
              "id": 815873,
              "name": "Sean Keys",
              "ops": 0.623,
              "avg": 0.176
            },
            {
              "id": 672960,
              "name": "Kazuma Okamoto",
              "ops": 0.707,
              "avg": 0.2
            },
            {
              "id": 669701,
              "name": "Josh Smith",
              "ops": 0.451,
              "avg": 0.083
            },
            {
              "id": 665926,
              "name": "Andrés Giménez",
              "ops": 0.623,
              "avg": 0.243
            },
            {
              "id": 676391,
              "name": "Ernie Clement",
              "ops": 0.501,
              "avg": 0.125
            }
          ],
          "bat_trend": "stable",
          "last5_rpg": 2.8,
          "last5_avg": 0.232,
          "home_split": {
            "recent_avg": 0.269,
            "runs_per_g": 5.1,
            "hr_per_g": 1.4,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.235,
            "runs_per_g": 2.5,
            "hr_per_g": 0.7,
            "n_games": 10
          }
        },
        "sit_score": 43.8,
        "total": 41.9
      },
      "home": {
        "sp_score": 36.6,
        "sp_detail": {
          "era": 3.79,
          "whip": 1.45,
          "k9": 6.6,
          "avg_ip": 5.5,
          "qs_rate": 30.0,
          "last3_era": 4.18,
          "last_start_era": 3.6,
          "last2_eras": [
            1.35,
            3.6
          ],
          "recent_avg_era": 3.6,
          "recent_bad_start": false,
          "trend": "cold",
          "n_games": 10,
          "sample_confidence": 1.0,
          "rest_days": 6,
          "rest_note": null,
          "age": 37,
          "fb_velo": 92.4,
          "pitch_arsenal": [
            "SI",
            "CB",
            "CT",
            "FB",
            "SW",
            "CH"
          ],
          "secondary_pitches": [
            "CB",
            "SW",
            "CH"
          ],
          "pitches_detail": [
            {
              "code": "SI",
              "abbr": "SI",
              "pct": 36.8,
              "velo": 92.3
            },
            {
              "code": "CU",
              "abbr": "CB",
              "pct": 21.5,
              "velo": 71.3
            },
            {
              "code": "FC",
              "abbr": "CT",
              "pct": 19.4,
              "velo": 90.0
            },
            {
              "code": "FF",
              "abbr": "FB",
              "pct": 8.6,
              "velo": 92.4
            },
            {
              "code": "ST",
              "abbr": "SW",
              "pct": 6.1,
              "velo": 73.5
            },
            {
              "code": "CH",
              "abbr": "CH",
              "pct": 4.8,
              "velo": 85.4
            }
          ]
        },
        "bp_score": 62.7,
        "bp_detail": {
          "bullpen_era": 3.58,
          "recent_era": 2.0,
          "recent_appearances": 18,
          "recent_ip": 17.0,
          "team_era": 3.97,
          "bp_ip": 374.7,
          "bp_count": 9,
          "closer_era": 3.54,
          "closer_name": "Andrew Kittredge"
        },
        "bat_score": 25.0,
        "bat_detail": {
          "recent_avg": 0.208,
          "runs_per_g": 3.4,
          "hr_per_g": 1.5,
          "bb_per_g": 3.0,
          "season_ops": 0.64,
          "season_slg": null,
          "season_avg": 0.208,
          "n_games": 9,
          "source": "prev_day",
          "handedness": "R",
          "splits_used": false,
          "lineup_ops": [
            "Dylan Beavers(0.494)",
            "Pete Alonso(0.952)",
            "Gunnar Henderson(0.796)",
            "Coby Mayo(0.772)",
            "Christian Encarnacion-Strand(0.944)",
            "Samuel Basallo(0.566)",
            "Rece Hinds(0.525)",
            "Colton Cowser(0.712)",
            "José Barrero(0.000)"
          ],
          "lineup_players": [
            {
              "id": 687637,
              "name": "Dylan Beavers",
              "ops": 0.494,
              "avg": 0.105
            },
            {
              "id": 624413,
              "name": "Pete Alonso",
              "ops": 0.952,
              "avg": 0.243
            },
            {
              "id": 683002,
              "name": "Gunnar Henderson",
              "ops": 0.796,
              "avg": 0.278
            },
            {
              "id": 691723,
              "name": "Coby Mayo",
              "ops": 0.772,
              "avg": 0.25
            },
            {
              "id": 687952,
              "name": "Christian Encarnacion-Strand",
              "ops": 0.944,
              "avg": 0.333
            },
            {
              "id": 694212,
              "name": "Samuel Basallo",
              "ops": 0.566,
              "avg": 0.138
            },
            {
              "id": 677956,
              "name": "Rece Hinds",
              "ops": 0.525,
              "avg": 0.2
            },
            {
              "id": 681297,
              "name": "Colton Cowser",
              "ops": 0.712,
              "avg": 0.321
            },
            {
              "id": 676480,
              "name": "José Barrero",
              "ops": 0.0,
              "avg": 0.0
            }
          ],
          "bat_trend": "cold",
          "last5_rpg": 2.4,
          "last5_avg": 0.168,
          "home_split": {
            "recent_avg": 0.212,
            "runs_per_g": 3.8,
            "hr_per_g": 1.0,
            "n_games": 10
          },
          "away_split": {
            "recent_avg": 0.225,
            "runs_per_g": 4.4,
            "hr_per_g": 1.6,
            "n_games": 10
          }
        },
        "sit_score": 47.8,
        "total": 46.1
      }
    },
    "scores": {
      "away_offense": 25.0,
      "away_defense": 49.8,
      "home_offense": 25.0,
      "home_defense": 49.7
    },
    "actual_score": {
      "away": null,
      "home": null
    },
    "lineup_confirmed": true,
    "sp_tbd": {
      "away": false,
      "home": false,
      "both": false,
      "any": false
    },
    "actual_winner": null,
    "model_winner": "Baltimore Orioles",
    "model_correct": null,
    "notes": "Toronto Blue Jays IL: Addison Barger, Anthony Santander, Bowden Francis, Cody Ponce, Dylan Cease 외 다수 / Baltimore Orioles IL: Blaze Alexander, Colin Selby, Jackson Holliday, Jordan Westburg, Keegan Akin 외 다수",
    "kalshi_prob": null,
    "edge": null,
    "value_bet": "마켓 없음",
    "extreme_edge": false,
    "consensus": false,
    "sp_bat_conflict": false,
    "sp_bat_conflict_detail": null,
    "low_confidence": true,
    "low_confidence_reason": "최고 확률 54.9% (≤58.0%)",
    "opener_game": {
      "away": false,
      "home": false
    },
    "model_version": "v15"
  }
];
